...
const AIRLINES = [
  ...
  // 티웨이항공 → 트리니티항공 사명 변경(2026-09-10 · 라벨 교체는 2026-09-22 오너 지시). 슬러그는 그대로(news.html·mypage 라벨 계약 + 저장된 기사의 값).
  // 옛 이름은 별칭·검색어로 남긴다(아래 QUERIES) — 옛 표기 기사·'구 티웨이' 병기 기사가 계속 나온다.
  { slug: 'twayair',   name: '트리니티항공', alias: ['트리니티항공', '트리니티 항공', '티웨이항공', '티웨이'] },
...
const STOCKS = /투자분석|투자의견|목표주가|목표가|적정주가|매수의견|매도의견|상한가|하한가|신고가|신저가|테마주|관련주|급등주|주가 전망|주가 흐름|기술적 분석|배당수익률|공매도|(?:대한항공|아시아나항공|제주항공)우(?![가-힣])/;
...
const TRAVEL_AD = /하나투어|모두투어|롯데관광개발|교원투어|노랑풍선|참좋은여행|한진트래블|인터파크투어|온라인투어|웹투어|여행박사|마이리얼트립|여기어때|야놀자|트립닷컴|스카이스캐너|익스피디아|아고다|부킹닷컴|투어비스|특가|초특가|땡처리|얼리버드|할인|프로모션|경품|사은품|증정|반값|이벤트|감사제|감사 행사|무료 제공|무료 지원|꿀팁|여행 팁|가지 팁|출국 팁|이용 팁|환승 팁|팁 소개|출국 노하우|가볼 만한|가볼만한|명소|맛집|호캉스|힐링 여행|여행지 추천|추천 여행지|피서지|패키지 (?:출시|선보|선봬|판매|공개)|여행상품 (?:출시|선보|판매)|전세기 상품|상품 출시/;
...
const ACADEMY = /학원|아카데미|합격(?:자|생)\s*배출|수강생|항공(?:서비스|운항|관광)\s*(?:학과|학부|과)\b|취업\s*연계|취업\s*역량|취업\s*캠프|\d개\s*대학/;
// ↑ 2026-09-21 첫 실데이터 dry-run 보강: '취업역량·취업 캠프·N개 대학' — "지역 3개 대학, 항공서비스 취업역량 강화 캠프"류
//   대학 홍보 3건이 빠져나갔다. ⚠️ '취업 준비'는 넣지 말 것 — 항공사의 '취업준비생 대상 설명회' 기사가 잘린다.
...
// ⚠️ 이 줄이 없으면 '진에어'·'에어프레미아'·'트리니티'처럼 이름에 '항공'이 없는 6개 항공사
...
// 수집 쿼리 = 항공사 10개 + 티웨이항공(트리니티항공의 옛 사명 — 옛 표기 기사) + 산업 일반 2개
const QUERIES = [...AIRLINES.map(a => a.name), '티웨이항공', '항공사 채용', '국내 항공업계'];
...
(async () => {
  ...
  for (const it of collected.values()) {
    const why = dropReason(it.title, it.url);
    if (why) {
  ...
  // ⚠️ url 을 같이 받아야 블로그 출처(호스트) 판정이 기존 저장분에도 걸린다.
  const stored = await sbFetchAll(
    'news_articles?select=id,title,url,source,airline,topic,published_at,news_scraps(id)');
  console.log(`저장분 ${stored.length}건 점검`);
  const purge = stored
    .filter(a => isDropped(a.title, a.url) && !(a.news_scraps || []).length)
    .map(a => a.id);
