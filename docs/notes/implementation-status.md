...
| sojae-chat | ⚠️ **레포 `2026-09-09a` — 아직 미배포(오너 콘솔 배포 대기).** 되묻기 상한 4→3 + 다듬기 `strengths` 를 문자열→목록(화면이 표·항목으로 그린다) + "한 답변에 질문 두 개 금지" 한 줄. **playbook `ask_core` 의 '4번'→'3번' SQL 도 같이 실행해야 한다**(대화창 제공). 미배포여도 화면은 안전하다 — 화면이 옛 문자열 `strengths` 도 목록으로 그리고, 상한은 화면 폴백이 3으로 돈다(다만 AI 는 배포 전까지 4문답으로 묻는다). 배포본 = **`2026-08-05a`**(2026-08-05 오너 배포·프로브 확인 — 4문답 상한 + 충분 시 새 질문 금지, playbook '6번'→'4번' SQL 실행됨) | `POST {probe:true}` → `version`·`features`·`playbook_keys`(9)·`has_api_key`. **`features` 에 `ask3`·`strength_list` 가 있으면 2026-09-09 판이 올라간 것** |
