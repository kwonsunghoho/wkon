...
| sojae-chat | ⚠️ **레포 `2026-09-09a` — 아직 미배포(오너 콘솔 배포 대기).** 되묻기 상한 4→3 + 다듬기 `strengths` 를 문자열→목록(화면이 표·항목으로 그린다) + "한 답변에 질문 두 개 금지" 한 줄. **playbook `ask_core` 의 숫자 SQL 도 같이 실행해야 한다**(대화창 제공 — 숫자 무관 `regexp_replace('[0-9]+번 주고받기','3번 주고받기')`). 미배포여도 화면은 안전하다 — 화면이 옛 문자열 `strengths` 도 목록으로 그리고, 상한은 화면 폴백이 3으로 돈다(다만 AI 는 배포 전까지 옛 상한으로 묻는다). 배포본 = **`2026-08-05a`**(2026-08-05 오너 배포·프로브 확인 — 4문답 상한 + 충분 시 새 질문 금지). ⚠️ **정정(2026-09-09 실측)**: 같은 날 실행했다고 적혀 있던 playbook `ask_core` '6번'→'4번' SQL 은 **실제로는 반영돼 있지 않았다** — 2026-09-09 SQL 반환값이 "잘 나와도 **6번** 주고받기 안에서 정리한다" 였다. 함수 ASK_FORMAT(4번)이 프롬프트 뒤에 붙어 이기긴 했지만, 한 프롬프트 안에 6과 4가 같이 있던 상태다. **playbook 수정 SQL 은 실행 여부를 반환값으로 확인한다 — '실행됨' 자기보고를 그대로 적지 말 것** | `POST {probe:true}` → `version`·`features`·`playbook_keys`(9)·`has_api_key`. **`features` 에 `ask3`·`strength_list` 가 있으면 2026-09-09 판이 올라간 것** |
