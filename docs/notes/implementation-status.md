...
| `20260824150000_blacklist` | **신청자 블랙리스트**(2026-08-24 오너 "우리만 알아볼 수 있게") — `blacklist`(phone 숫자만 unique · name · reason). RLS 는 `is_admin()` **하나뿐**(본인 포함 조회 창구 금지). admin 신청자 현황에 라벨·등록/해제 버튼 — 표시만, 신청·결제는 안 막는다 | **⚠️ owner 실행 필요(2026-08-24 작성 · SQL 대화창 전달).** 미적용이어도 신청자 현황은 그대로 돈다 — 라벨만 안 뜨고, [블랙 등록] 클릭 시 실행 안내가 뜬다(PGRST205 판정) |
