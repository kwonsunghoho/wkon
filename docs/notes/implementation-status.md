...
| `20260805160000_member_course` | **승준노트 허브 코스** — `members.course`(beginner/practical/spurt/daily · null=미선택) + 체크 제약. 새 RLS 없음(기존 `members_update_own` 이 덮는다) | **owner 실행 필요(2026-08-05 작성)** — 미적용이면 허브가 '코스 선택 전' 화면만 보여 주고, [이 코스로 시작하기] 를 누르면 "지금은 코스 저장이 안 돼요" 를 그 자리에 띄운다(성공한 척 넘어가지 않는다). 도구 링크·서클·무료 맛보기는 그대로 동작. 조회는 `select('*')` 라 400 이 안 난다. 다른 표를 안 건드리므로 실행 순서 무관 |
