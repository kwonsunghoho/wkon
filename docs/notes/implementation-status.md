...
| `20260904130000_culture_round_constraint` | **challenge_rounds 가 'culture' 기수를 받게**(2026-09-04 실사고 — admin 기수 저장이 `challenge_rounds_challenge_check` 위반으로 막혔다). `challenge_rounds` 는 콘솔 생성 표라 레포에 정의가 없어 챌린지 값 목록 제약이 DB 에만 있었다. 제약을 없애지 않고 목록만 넓힌다(오타 기수를 잡아 주는 안전장치다) | **미적용(오너 실행 대기 · 이걸 해야 댄특완 기수를 만들 수 있다).** ⚠️ 앞으로 챌린지를 더 만들면 이 목록에 id 를 더한다 |
| `20260904120000_culture_lessons` | **댄.특.완 DAY 문항 10줄 심기**(2026-09-04 챌린지 신설) — `challenge_lessons` 에 `challenge='culture'` 행만 넣는다(표·RLS 는 20260824120000 이 이미 만들었다). 읽는 곳: challenge-culture·apply·mypage·submit, 고치는 곳: admin '챌린지' 탭 → DAY 문항 고치기 → 댄특완 | **미적용(오너 실행 대기).** 미적용이어도 네 화면은 HTML 폴백 10줄로 그대로 돈다 — 다만 admin 에서 문항을 고칠 수 없다(목록이 비어 '심기 구문을 실행하세요'). ⚠️ `match_words` 칸은 20260824160000 에서 없앴다 — 다시 넣지 말 것 |
