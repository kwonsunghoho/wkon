...
| sojae-chat | 배포 `2026-08-04a` / **레포 `2026-08-05a`(4문답 상한 + 충분 시 새 질문 금지) — 오너 콘솔 배포 필요.** 재료 게이지(화면)는 `missing` 이 이미 배포돼 있어 구버전에서도 동작 | `POST {probe:true}` → `version` 이 `2026-08-05a` 면 반영됨 |
