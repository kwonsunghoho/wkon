...
| verify-identity | **⚠️ 미배포(2026-08-26 신설 — 오너 콘솔 배포 대기).** 휴대폰 본인인증(KG이니시스 통합인증 · 포트원 경유) — 온보딩이 보낸 `identityVerificationId` 를 포트원 API 로 재조회(`status==='VERIFIED'`)한 뒤 RPC `apply_identity_verification`(service_role 전용 · migration `20260826150000`)으로 실명·번호·CI·DI 저장 + CI·번호 중복 판정. 환경변수는 기존 `PORTONE_API_SECRET` 그대로(추가 설정 없음). **미배포여도 안전** — 온보딩이 프로브로 확인한 뒤에만 인증 UI 를 켠다(그전엔 직접 입력 폼) | `POST {"probe":true}` → `version` `2026-08-26a`. 404 면 미배포. 마이그레이션 미적용이면 본 호출이 `not_ready`(화면은 직접 입력 폴백) |
