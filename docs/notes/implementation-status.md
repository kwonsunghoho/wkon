...
| `20260822170000_member_notes_tags` | **회원 메모·태그**(admin 회원 관리 2단계) — `member_notes`(회원당 여러 줄·시간순) + `member_tags`(회원당 여러 개·자유 입력 20자). RLS 는 `is_admin()` **하나뿐** | **⚠️ owner 실행 필요(2026-08-23 작성).** 미실행이면 회원 상세의 '메모·태그' 패널만 안내 문구로 바뀌고 나머지(현황 5단·분포·활동 타임라인)는 그대로 돈다 — 목 프리뷰로 실측 확인. ⚠️ **`authenticated` 용 select 정책을 만들지 말 것** — 회원 본인이 자기 상담 메모를 읽는다. ⚠️ 이 표의 **내용은 개인정보다**(값 insert 를 레포에 커밋 금지) |
| `20260822180000_member_segments` | **회원 조건 저장(세그먼트)**(3단계) — `member_segments`(name · rule jsonb `{"all":[…]}` AND 만 · followup). 이름은 대소문자·공백 무시 유일 인덱스. 거르기는 브라우저가 한다(이 표는 조건만 담는다) | **⚠️ owner 실행 필요(2026-08-23 작성).** 미실행이면 셀렉트의 '저장한 조건' 묶음만 통째로 숨고, '조건 더 걸기'(그때그때 거르기)·현황·타임라인은 그대로 돈다. '오늘' 탭 후속 조치 줄도 안 뜬다 |
