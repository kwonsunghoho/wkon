...
| ai-killer (polish·quickfix 겸용) | 2026-07-25 확인 `2026-07-25d`·프로필 4곳·사전 28개 → polish 추가 후 `2026-07-30b`(`coach_terms` 주입). **레포는 `2026-08-04a` — 재배포 필요**(미니 다듬기 `quickfix` + 환급 서버 전용화 `refund_server`) | 로그인 없이 `POST {"probe":true}` → `version`·`features`·`airline_profiles`/`terms` 개수·`has_api_key`. `features` 에 `quickfix` + `quickfix_table` 숫자면 미니 다듬기 살아 있음. **`refund_server` 있으면 환급이 service_role 경로**(없으면 구버전 — 20260804160000 을 실행하면 안 된다). `coach_terms` 0 이면 연구진 자산 미유입 |
| sojae-chat | v2(카드+뼈대) 배포. **크레딧 차감은 재배포된 버전부터 동작** — 재배포 전엔 화면 안내만 바뀌고 차감 안 됨. **레포는 `2026-08-04a` — 재배포 필요**(환급 서버 전용화) | `POST {probe:true}` → `version`·`playbook_keys`·`features`. **`features` 에 `refund_server` 있으면 새 버전** |
...
| `20260804150000_rls_hardening` | **RLS 하드닝 3종**(2026-08-04 보안 점검) — ① 서버 전용 환급 `refund_credit_for(uuid,text,text)`(service_role 만) ② `applications` INSERT 를 '결제 전 상태'로 제한(paid·payment_status·payment_id·paid_amount·refunded 고정) ③ 콘솔 생성 표 `reviews`·`challenge_rounds` RLS + 정책 | **owner 실행 필요(2026-08-04 작성)** — 이 파일 하나로는 아무것도 안 깨진다(기존 함수·정책을 없애지 않는다). ⚠️ **①→에지 함수 재배포→`20260804160000` 순서를 지킨다.** 미적용 상태에서 새 에지 함수를 배포하면 AI 실패 시 환급이 죽는다(반대로 이것만 적용하고 구버전 함수가 돌아도 정상 — 구 RPC 가 아직 살아 있다). ⚠️ 적용 후 `pg_policies` 로 `reviews`·`challenge_rounds` 에 **이름 다른 옛 정책**이 남아 있는지 확인(정책은 OR 로 합쳐져 헐거운 쪽이 이긴다) |
| `20260804160000_refund_credit_lockdown` | 구 `refund_credit(text,text)` 의 `authenticated` 실행 권한 회수 | **owner 실행 필요 — 반드시 마지막.** 선행 조건 둘: `20260804150000` 적용 + ai-killer·sojae-chat 프로브 `features` 에 `refund_server` 확인. 순서를 어기면 AI 호출 실패 시 환급이 권한 없음으로 죽어 **학생이 결과도 못 받고 크레딧만 잃는다** |
