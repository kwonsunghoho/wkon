...
| portone-webhook | **신설 `2026-08-10a` — owner 배포 + 설정 2가지 필요(2026-08-10 작성).** 포트원 결제 통보를 받아 브라우저 없이 지급을 끝내는 최종 안전망(유실 0%). ① 콘솔에서 새 함수 `portone-webhook` 생성·배포 후 **그 함수만 Verify JWT 끄기** ② **포트원 콘솔 > 웹훅에 URL 등록**: `https://apzwauiumhmsvrgffjis.supabase.co/functions/v1/portone-webhook` (V2). 선택: 포트원 웹훅 시크릿을 Supabase Secrets `PORTONE_WEBHOOK_SECRET` 에 넣으면 서명까지 검증(없어도 안전 — 본문 불신·API 재조회). ⚠️ 결제 생성 때 실은 `customData.k` 가 있어야 지급한다 — 이 배포(사이트 2026-08-10) 이후 결제부터 적용, 옛 결제는 기존 경로가 처리 | `POST {"probe":true}` → `version`·`has_secret`. 404=미배포. 실전 확인: 결제 하나 하고 포트원 콘솔 웹훅 발송 이력에서 200 응답 확인 |
