...
- **카카오페이 채널 키(2026-08-19)** — 가맹 심사는 완료. 결제 7곳에 카카오페이 코드가 들어갔지만
  `pay-methods.js` 의 `kakao` 키가 빈 문자열이라 **화면에는 아직 안 나온다**(토스 단독 — 종전과 동일).
  오너가 포트원 콘솔(연동 관리 → 채널 관리 → 채널 추가 → 카카오페이·실연동·CID 입력)에서 채널을
  만들어 채널 키를 주면, `pay-methods.js` 한 곳만 채우고 7개 페이지 `?v=` 올려 배포한다.
  배포 후 최종 확인은 카카오페이 소액 실결제 1건 + admin 환불. 서버 함수는 무변경(채널 무관).
