...
| migration | 내용 | 기록된 상태 |
|---|---|---|
| `20260909120000_academy_students` | **재학생 무료 참여**(2026-09-09 오너 지시) — 명단 표 `academy_students`(관리자 전용) + 판정 `monc_is_student()`·`my_student_status()` + 무료 접수 3경로 `apply_free_challenges`·`apply_free_lecture`·`claim_free_resource`. 번호 값은 레포에 넣지 않는다 — 등록은 admin '회원 관리 > 재학생 명단'에서 | **미적용 — 오너가 SQL Editor 에서 실행해야 켜진다.** 미적용 상태 degrade: 화면이 `my_student_status()` 를 못 찾아 전원 유료(지금과 동일), admin '재학생 명단' 묶음만 안내로 바뀐다. 확인: 관리자 계정으로 `select public.my_student_status();` 가 `{"student":false,…}` 를 돌려주면 적용됨 |
