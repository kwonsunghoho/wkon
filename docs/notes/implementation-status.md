...
| `20260805130000_lecture_login_required` | **특강 신청 로그인 필수**(2026-08-05 오너 확정) — `applications_insert_public` 에 `(lecture_id is null or member_id = auth.uid())` 추가. 챌린지는 비회원 신청 유지, 특강만 로그인 본인 | **owner 실행 필요(2026-08-05 작성)** — 선행: `20260804150000` 적용. 미적용이면 특강도 비회원 신청이 되지만 화면·verify-payment 두 겹은 이미 막는다(RLS 는 세 번째 겹). ⚠️ **verify-payment 도 재배포해야 완성**(lectureId 결제 JWT 요구 + member_id 를 JWT 로 결정). 재배포 확인: 비로그인 프로브 `{paymentId:'probe', lectureId:'0000…'}` → **`login_required`**=신버전(구버전은 `lecture_not_found`). lecture.html 은 라이브 직행이라 이미 반영됨 |
