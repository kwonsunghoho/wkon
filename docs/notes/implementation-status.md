...
- **연구실 자료별 미리보기 자동 실행(2026-09-21) — 가동.** 오너가 GitHub 웹에서 `.github/workflows/lab-og.yml` 을
  추가했다(`c6680fb` · 설계본과 내용 동일 확인 · Actions 목록에 `active`). 이 기기의 GitHub 토큰에는 workflow 권한이
  없다 — **이 파일을 고칠 일이 생기면 또 오너가 웹에서 고쳐야 한다**(푸시가 거절된다).
  남은 확인 하나: 자동 커밋이 Pages 배포까지 이어지는지(새 자료를 올린 뒤 1시간 안에 `r/<8자>.html` 이 열리는지).
...
| answer-program | **레포 `2026-09-22a`(티웨이항공 → 트리니티항공 라벨만 · 2026-09-22) — 미배포. `2026-08-16b` 배포 완료(2026-08-16 오너 배포 — 프로브 실측: features 에 `card_from_chat`, prompt `ap-2026-08-16a`, questions 99 · programs 1)** — 소재 창고 개명 문구 + 소재 발굴 연동 포함 | 프로브 있음 — 상세는 `docs/monc-answer-program/implementation-status.md` |
