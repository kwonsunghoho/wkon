...
| `20260824160000_drop_lesson_match_words` | **문항 판정 낱말 칸 제거**(2026-08-24 오너 "기능 없애버려 헷갈려") — `challenge_lessons.match_words` drop. 판정은 **문항 이름에서 만든다**(mypage `wordsOf()`) | **⚠️ owner 실행 필요(2026-08-24 작성).** 미실행이어도 안 깨진다 — 화면이 이 컬럼을 더는 읽지도 쓰지도 않는다. 안 쓰는 칸이 남아 헷갈리므로 지우는 게 맞다 |
