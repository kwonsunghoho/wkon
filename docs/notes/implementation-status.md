...
| `20260819130000_lecture_show_seats` | **특강 잔여석 숨기기 스위치** — `special_lectures.show_seats`(boolean · 기본 true). 표시 전용 컬럼이라 RLS·트리거는 안 건드린다 | **owner 실행 필요(2026-08-19 작성)** — 미적용이면 잔여석이 지금까지처럼 늘 보이고(`show_seats` 가 없으면 화면은 '보이기'로 읽는다), admin 에서 스위치를 끄고 저장하면 특강은 저장되되 '잔여석 공개 설정은 저장되지 않았어요' 안내가 뜬다(`42703`/`PGRST204` 판정 후 그 칸만 빼고 재시도). 목록 조회는 `select('*')` 라 400 이 안 난다. 다른 표를 안 건드리므로 실행 순서 무관 |
