...
- **연구실 자료별 미리보기 자동 실행(2026-09-21) — 오너가 GitHub 웹에서 파일 하나를 추가해야 가동된다.**
  생성기(`scripts/lab-og-stubs.mjs`)·첫 27건(`r/`)·admin [링크] 연결은 main 에 올라가 있다. 남은 것은
  1시간 주기 실행 파일 `.github/workflows/lab-og.yml` 하나 — **이 기기의 GitHub 토큰에 workflow 권한이 없어
  푸시가 거절됐다**(`refusing to allow a Personal Access Token to create or update workflow`). 파일 내용은
  대화창으로 전달했다(lab.md '자료별 미리보기' 절의 설계 그대로). 추가 전까지는 새 자료의 안내판이
  자동으로 안 생긴다 — 그동안 [링크]는 서가 공용 미리보기 주소(`lab-<서가>.html?r=`)를 복사하므로 링크 자체는 열린다.
  추가했으면 이 항목을 지운다.

- **카카오페이 결제(2026-08-19) — 가동 확인 완료.** 채널 키 반영·배포 후 **오너 실결제 1건으로
  검증 완료(2026-08-19 "카카오페이 잘 되네")**. 테스트 결제 환불만 오너 몫으로 남음.
  키를 비우면 카카오페이가 화면에서 통째로 꺼진다 — 문제가 생기면 그게 끄는 스위치다.
  서버 함수는 무변경(채널 무관 — 금액은 포트원+DB 재확인). 상세는 apply-and-payment.md 맨 위 절.
...
| portone-webhook | **레포 `2026-09-18a` 미배포(테스트 채널 결제 지급 건너뛰기 한 줄).** 이전 기록: **`2026-08-10a` 가동 중 — 실결제 검증까지 완료(2026-08-10).** ① 배포 + Verify JWT 꺼짐 — 키 없는 브라우저 프로브 실측 확인 ② **포트원 콘솔 웹훅 URL 등록 완료**(실연동 · application/json · 버전 2024-04-25) ③ **오너 테스트 실결제 1건 — 포트원 콘솔 웹훅 발송 이력 전부 200 응답 실측**. 포트원 결제 통보를 받아 브라우저 없이 지급을 끝내는 최종 안전망 — 이로써 지급 경로 3겹(브라우저 확인 → 자가 회복 → 웹훅)이 전부 가동. 선택: 포트원 웹훅 시크릿을 Supabase Secrets `PORTONE_WEBHOOK_SECRET` 에 넣으면 서명까지 검증(없어도 안전 — 본문 불신·API 재조회). ⚠️ 결제 생성 때 실은 `customData.k` 가 있어야 지급한다 — 사이트 2026-08-10 배포 이후 결제부터 적용, 옛 결제는 기존 경로가 처리 | `POST {"probe":true}` → `version`·`has_secret` (**키 없이 호출해야 한다** — 401 이 오면 Verify JWT 가 다시 켜진 것). **실전 확인 완료(2026-08-10): 테스트 결제 1건, 포트원 콘솔 웹훅 발송 이력 전부 200** |
| ai-killer (polish·quickfix 겸용) | **레포 = 배포본 = `2026-08-22a`(첨삭 분량 전환 + N초 버전 `polish_length_version`) — 2026-08-22 오너 배포("함수 배포했어") · anon 프로브 실측: `version` `2026-08-22a` · features 에 `polish_length_version` · `polish_table` 42 · `quickfix_table` 24 · `has_api_key` true. 화면(main `bc83800`)도 monc.ai.kr 반영 실측(polish.html 에 `rsLenBtns` 있음).** 앞선 `2026-08-21a`(첨삭 말하기 분량 옵션 `polish_length`)는 오너 배포 보고(2026-08-21 "함수 배포하고 병합했어"). ⚠️ 이 세션 프록시가 supabase.co 를 막아(403) 프로브 실측은 못 했다** — 의심되면 로그인 후 polish.html 에서 '말하기 분량' 줄이 보이는지 본다(그 칸은 프로브 features 의 `polish_length` 를 받아야만 그려진다 — 안 보이면 구버전). 화면(main `7b43a77`)은 2026-08-21 Pages 배포 성공 실측. 미배포여도 안전하다 — 배포 전에는 분량 칸이 안 떠서 화면이 종전과 똑같다. **배포본 = `2026-08-14` — 배포 완료(2026-08-14 오너 배포 · anon 프로브 실측: `version` `2026-08-14` · `polish_table` 19 · `quickfix_table` 15 · `has_api_key` true).** 2026-08-14 **첨삭(polish) 지침 4줄** 추가 — 진단 우선 · fix 완결 문장 · rewrites 고르는 순서 · 코치 말투(제미나이 대조 후 넣었다 · 배경과 되돌리면 안 되는 이유는 `docs/notes/polish.md`). 앞선 이력: **`2026-08-12b` 배포 확인(2026-08-13 프로브 — `holistic`·`green_flags`)** — 킬러 판정을 규칙 엔진에서 **오너 지침 종합 판정(4기준 + AI 의심 지수 %)** 으로 전면 교체 + **점수 보정·그린 플래그** 지침 추가. 판정 피드백(만족/보통/불만족, 2026-08-13)은 **함수와 무관** — 브라우저가 `ai_killer_feedback` 에 직접 쓴다(재배포 불필요). ⚠️ **첨삭 지침은 `features` 에 안 잡힌다 — 배포 확인은 `version` 으로 본다**(`holistic`·`green_flags` 는 `2026-08-12b` 에도 있어서 옛 버전이 올라가 있어도 통과한다) | 로그인 없이 `POST {"probe":true}` → `version`·`features`·`airline_profiles`/`terms` 개수·`has_api_key`. `features` 에 `quickfix` + `quickfix_table` 숫자면 미니 다듬기 살아 있음. **`refund_server` 있으면 환급이 service_role 경로**(없으면 구버전 — 20260804160000 을 실행하면 안 된다). `coach_terms` 0 이면 연구진 자산 미유입. ⚠️ 오너 환경에선 크롬이 콘솔 붙여넣기를 막는다("allow pasting" 을 직접 타이핑해야 한다) — **콘솔 코드 화면에서 `FN_VERSION` 을 눈으로 보는 쪽이 빠르다** |
| sojae-chat | **`2026-09-09a` 배포 확인(2026-09-09 anon 프로브 실측 — `version` `2026-09-09a` · features 에 `ask3`·`strength_list` · `playbook_keys` 9 · `materials_table` 204 · `has_api_key` true). playbook 숫자 SQL 도 같은 날 실행 완료(반환값 "3번 주고받기" 실측). 화면도 monc.ai.kr 반영 실측(`sojae.html` 에 '방금 보낸 말 고쳐 쓰기'·`sk-tbl` 있음). 레포와 같음.** 되묻기 상한 4→3 + 다듬기 `strengths` 를 문자열→목록(화면이 표·항목으로 그린다) + "한 답변에 질문 두 개 금지" 한 줄. **playbook `ask_core` 의 숫자 SQL 도 같이 실행해야 한다**(대화창 제공 — 숫자 무관 `regexp_replace('[0-9]+번 주고받기','3번 주고받기')`). 화면은 옛 문자열 `strengths` 도 목록으로 그린다(구버전 함수 호환 — 되돌리기 안전망). 배포본 = **`2026-08-05a`**(2026-08-05 오너 배포·프로브 확인 — 4문답 상한 + 충분 시 새 질문 금지). ⚠️ **정정(2026-09-09 실측)**: 같은 날 실행했다고 적혀 있던 playbook `ask_core` '6번'→'4번' SQL 은 **실제로는 반영돼 있지 않았다** — 2026-09-09 SQL 반환값이 "잘 나와도 **6번** 주고받기 안에서 정리한다" 였다. 함수 ASK_FORMAT(4번)이 프롬프트 뒤에 붙어 이기긴 했지만, 한 프롬프트 안에 6과 4가 같이 있던 상태다. **playbook 수정 SQL 은 실행 여부를 반환값으로 확인한다 — '실행됨' 자기보고를 그대로 적지 말 것** | `POST {probe:true}` → `version`·`features`·`playbook_keys`(9)·`has_api_key`. **`features` 에 `ask3`·`strength_list` 가 있으면 2026-09-09 판이 올라간 것** |
| answer-program | **`2026-08-16b` 배포 완료(2026-08-16 오너 배포 — 프로브 실측: features 에 `card_from_chat`, prompt `ap-2026-08-16a`, questions 99 · programs 1)** — 소재 창고 개명 문구 + 소재 발굴 연동 포함 | 프로브 있음 — 상세는 `docs/monc-answer-program/implementation-status.md` |
| verify-identity | **`2026-08-26b` 배포 확인(2026-08-27 anon 프로브 실측: `version` `2026-08-26b` — 오너가 콘솔 배포 완료)**: 실사고 "인증 확인에 실패했어요" 추적용 — ① 인증사 번호 +82 국제형·하이픈 정규화 ② 번호 누락(통합인증 창에서 카드·간편인증서 방식 선택 시)이면 bad_phone + verifiedCustomer 키 로그 ③ 성공 아닌 결과 전부 로그. 앞선 배포본 `2026-08-26a` 는 2026-08-26 오너 실기기 인증 완주 확인("인증해봤는데 잘 되네") — 인증창→서버 검증→저장까지 전 구간 실전 검증 완료. 휴대폰 본인인증(KG이니시스 통합인증 · 포트원 경유) — 온보딩이 보낸 `identityVerificationId` 를 포트원 API 로 재조회(`status==='VERIFIED'`)한 뒤 RPC `apply_identity_verification`(service_role 전용 · migration `20260826150000`)으로 실명·번호·CI·DI 저장 + CI·번호 중복 판정. 환경변수는 기존 `PORTONE_API_SECRET` 그대로(추가 설정 없음). 미배포여도 안전 — 온보딩이 프로브로 확인한 뒤에만 인증 UI 를 켠다(그전엔 직접 입력 폼) | `POST {"probe":true}` → `version` `2026-08-26a`. 404 면 미배포. 마이그레이션 미적용이면 본 호출이 `not_ready`(화면은 직접 입력 폴백) |
| lab-file | **`2026-08-10a` 배포(2026-08-10 오너 배포 보고 — 이 세션 프록시가 supabase.co 를 막아(403) 프로브 실측은 못 했다. 의심되면 프로브 또는 admin 계정으로 유료 자료를 눌러 결제창이 안 뜨는지 확인).** 이 판은 관리자(`members.role='admin'`) 무료 열람을 넣었다(`features` 에 `admin_free`). 앞선 이력: **`2026-08-01f` 배포 확인**(프로브 실측 2026-08-01 — `features` 에 `paid`+`multi_file`) | `POST {"probe":true}` → `version`·`features`·`bucket`. `features` 에 `external_url`=영상 지원, **`paid`=유료 지원(e), `multi_file`=자료 하나에 파일 여러 개(f)**. 404 면 미배포. ⚠️ **`paid` 없는 버전에서 자료에 값을 매기면 무료로 열리고, `multi_file` 없는 버전은 파일이 여럿이어도 첫 파일만 내보낸다**(2026-08-01 실사고 — 마이그레이션과 재배포를 같이 한다) |
...
| migration | 내용 | 기록된 상태 |
|---|---|---|
| `20260916120000_academy_student_requests` | **재학생 자가 신청 → 승인**(2026-09-16 오너 지시) — `academy_students` 에 `status(pending/approved)`·`member_id`·`requested_at`, `valid_until` nullable. `monc_is_student`·`my_student_status` 를 approved 기준으로 교체(+`status`·`verified` 반환), 신청 RPC `request_student_status`(인증 회원만) | **적용 완료(2026-09-16 오너 실행 · anon 프로브 실측: `rpc/request_student_status` → `42501`, `academy_students?select=status` → 200).** 미적용 degrade 는 마이페이지 신청 버튼이 `not_ready` 안내로 멈추고, admin 은 대기 줄 없이 종전 목록. 프로브: anon 으로 `rpc/request_student_status` 에 `{}` → `42501`=적용, `PGRST202`=미적용 |
| `20260909120000_academy_students` | **재학생 무료 참여**(2026-09-09 오너 지시) — 명단 표 `academy_students`(관리자 전용) + 판정 `monc_is_student()`·`my_student_status()` + 무료 접수 3경로 `apply_free_challenges`·`apply_free_lecture`·`claim_free_resource`. 번호 값은 레포에 넣지 않는다 — 등록은 admin '회원 관리 > 재학생 명단'에서 | **적용 완료(2026-09-16 오너 실행 · anon 프로브 실측: 함수 5개 전부 `42501`(존재하지만 anon 금지 — 설계대로) · `academy_students` anon 조회 `*/0`).** 미적용 상태 degrade 는 화면이 `my_student_status()` 를 못 찾아 전원 유료, admin '재학생 명단' 묶음만 안내. 프로브: anon 으로 `rpc/my_student_status` 에 `{}` → `42501`=적용, `PGRST202`=미적용 |
| `20260908120000_review_culture` | **회원 직접 후기 제출에 댄특완(culture) 합류**(2026-09-08) — `submit_challenge_review` 의 챌린지 id → 한글값 case 에 `culture → '댄특완'` 한 줄. 후기 직접 제출은 2026-08-20, 댄특완은 2026-09-04 신설이라 빠져 있었다(시작일 조건이 아니라 누락). 원판은 20260822140000(부분 환불 반영본) — 기수 조회의 `partial_refunded` 를 그대로 들고 왔다 | **적용 완료(2026-09-08 오너 실행 · "완료").** 화면 세 곳(review-write `CH_ORDER`·admin `REVIEW_CH`·reviews-list 필터)도 같은 날 main 배포. ⚠️ **밖에서 확인할 수 있는 것은 함수 존재까지다** — anon 프로브는 `42501`(권한 없음)만 돌려주고, culture 분기는 로그인 참가자 계정으로만 확인된다. 첫 댄특완 후기 제출이 실제 확인이다 |
| `20260904130000_culture_round_constraint` | **challenge_rounds 가 'culture' 기수를 받게**(2026-09-04 실사고 — admin 기수 저장이 `challenge_rounds_challenge_check` 위반으로 막혔다). `challenge_rounds` 는 콘솔 생성 표라 레포에 정의가 없어 챌린지 값 목록 제약이 DB 에만 있었다. 제약을 없애지 않고 목록만 넓힌다(오타 기수를 잡아 주는 안전장치다) | **적용 완료(2026-09-08 오너 실행).** 확인: anon 조회로 `challenge_rounds` 에 댄특완 1기 행이 존재(모집 2026-09-04~09-20 · 선착순). ⚠️ 앞으로 챌린지를 더 만들면 이 목록에 id 를 더한다 |
| `20260904120000_culture_lessons` | **댄.특.완 DAY 문항 10줄 심기**(2026-09-04 챌린지 신설) — `challenge_lessons` 에 `challenge='culture'` 행만 넣는다(표·RLS 는 20260824120000 이 이미 만들었다). 읽는 곳: challenge-culture·apply·mypage·submit, 고치는 곳: admin '챌린지' 탭 → DAY 문항 고치기 → 댄특완 | **적용 완료(2026-09-08 오너 실행).** 확인: anon 조회로 `challenge='culture'` 10줄(DAY1 '좋은 신입은…' ~ DAY10 '대한항공 10년 후…'). ⚠️ 9일차 원문의 곡선 따옴표(‘ ’)는 SQL 안전을 위해 작은따옴표로 바꿔 심었다 — 되돌리려면 admin 문항 편집에서. ⚠️ `match_words` 칸은 20260824160000 에서 없앴다 — 다시 넣지 말 것 |
...
| `20260801180000_reviews_kind` | **후기 종류 분리** — `reviews.kind`(challenge/consult) + `image_path` not null 완화(글만 있는 후기) + `success_stories`(합격 수기) 표·RLS | **적용 확인(2026-08-20 anon 프로브 실측: `reviews?select=kind` HTTP 200 값 반환 · `success_stories` HTTP 200 `[]`. 그전까지 'owner 실행 필요'로 적혀 있었으나 실측과 어긋났다 — 다시 실행 안내 금지)** |
...
| `20260912120000_lecture_price_kind` | **특강 금액 이름(참가비/예약금)** — `special_lectures.price_kind`(text · `fee` 기본 | `deposit` · check 제약). 예약금이면 카드·상세·신청 폼 라벨이 '예약금' + '잔금은 별도로 안내드려요' 한 줄. 표시 전용 — 청구 금액은 `price` 그대로 | **미적용(2026-09-12 작성)** — 오너가 SQL Editor 에서 실행. 미적용이면 '참가비'로 동작하고 admin 저장은 그 컬럼만 빼고 저장(`LEC_OPT_COLS` 재시도) |
...
| `20260820170000_phone_dedup` | **전화번호 중복 가입 차단** — `save_my_profile()` RPC(온보딩·마이페이지 프로필 저장 단일 경로 · 같은 정규화 번호의 다른 회원이 있으면 `dup_phone`+기존 계정 provider 반환 · 24시간 10회 제한) + `phone_save_attempts` 표(RLS 잠금) + `members` 번호 정규화 인덱스. **유니크 인덱스는 일부러 없음**(기존 중복 데이터가 있으면 생성 실패 — 판정은 RPC 한 곳) | **객체 전부 성립(2026-08-26 — 이 파일 자체가 아니라 `20260826150000` 실행으로. anon 프로브 실측: RPC `save_my_profile` 401 `42501`=존재+차단).** `save_my_profile` 은 20260826 신판(본인인증 잠금 포함)이 최종 — **이 파일을 이제 와서 실행하면 잠금이 사라진다(아래 동명 함수 재실행 경고)** |
| `20260826150000_identity_verification` | **휴대폰 본인인증**(KG이니시스 통합인증 · 포트원 경유) — `members.ci·di·birth_date·verified_at` + `identity_verifications` 감사 표(RLS 잠금 · verification_id UNIQUE 로 재사용 차단) + `apply_identity_verification()`(**service_role 전용 — authenticated grant 절대 금지**) + `save_my_profile()` 재정의(verified 회원 name·phone 잠금 — **이 파일이 최종 정의**, 20260820170000 을 이 뒤에 재실행 금지). 20260820170000 미적용 환경에서도 홀로 성립(선행분 재선언) | **적용 완료(2026-08-26 오너 실행 — anon 프로브 실측 5종: 함수 프로브 `2026-08-26a` · `members?select=verified_at` 200 · `identity_verifications` 200 `[]`(존재+RLS 잠금) · RPC `apply_identity_verification`/`save_my_profile` anon 호출 401 `42501` permission denied(존재+차단 — 미적용이면 404 PGRST202). 다시 실행 안내 금지).** 이 실행으로 선행 20260820170000 분(save_my_profile·phone_save_attempts)도 함께 성립 |
...
| `20260822140000_participant_partial_refund` | **챌린지 참가 판정 — 부분 환불 포함**(감사 #2) — `is_challenge_participant()` 와 **`submit_challenge_review()` 기수 조회**의 통과 목록에 `partial_refunded` 추가(전액 환불만 배제 — 한쪽만 고치면 참가 판정은 통과하는데 후기 기수가 미상으로 저장된다). 화면(mypage 제출 카드·review-write)과 한 기준이 된다. 다건 챌린지 결제 후 1건만 환불한 회원의 나머지 챌린지 제출 차단도 함께 풀린다 | **⚠️ owner 실행 필요(2026-08-22 작성).** 미실행이면 부분 환불(중도 해지) 회원의 제출 업로드가 42501, 후기가 not_participant 로 실패 |
| `20260822150000_hidden_visible_enrolled` | **숨김 특강·프로그램을 신청·등록 본인에게는 보이게**(감사 #5) — definer 판정 함수 2개(`monc_applied_lecture`·`monc_enrolled_program` — 프로그램은 **active 이용권만**, ap_program_view RPC 와 같은 기준: 만료·환불 회원까지 열면 RPC 가 not_found 를 주는 막다른 카드가 뜬다) + 두 select 정책에 `or 본인` 추가(anon 은 여전히 visible 만). 같은 커밋에서 programs.html 비공개 배지 문구 중립화 + **mypage '다가오는 일정' 조회의 죽은 컬럼(schedule→select('*'))도 수리**(그전엔 400 으로 카드가 아예 안 그려졌다) | **⚠️ owner 실행 필요(2026-08-22 작성).** 미실행이면 판매 종료로 숨긴 특강·프로그램이 신청자·수강생 mypage·허브에서 조용히 사라진다(0행 — 에러 없음) |
...
| `20260821120000_sojae_access_revert` | **소재 권한 게이트 잔재 제거(답변집 저장 사고 수리)** — `discovery_sessions`·`discovery_messages`·`answers` 의 `_own` 정책에서 `can_sojae()` 조건을 빼고 원래 정의(본인 행이면 CRUD)로 원복 + `can_sojae()` 함수·`members.sojae_enabled` 컬럼 삭제 | **적용 완료(2026-08-23 anon 프로브 실측 — `members?select=sojae_enabled` 400 `42703` "column members.sojae_enabled does not exist"(컬럼 삭제됨) + `rpc/can_sojae` 404 `PGRST202`(함수 삭제됨) + 대조군 `members?select=id` 200 `[]`. 다시 실행 안내 금지).** 아래는 적용 전 상태 기록이다 —  2026-07-27 폐지 때 화면·서버 검사만 지우고 **RLS 되돌리기가 빠져**, `sojae_enabled=false` 회원(폐지 뒤 가입자 전원 — 켜 줄 수단이 없다)은 답변집 저장이 42501 "row-level security policy for table \"answers\"" 로 계속 실패하고 목록도 빈 화면이다(2026-08-21 학생 캡처 실증). 적용 전까지는 그 회원들의 소재 발굴 저장·직접 쓰기·프로그램 확정본이 전부 막혀 있었다. 적용 뒤에는 재로그인 없이 바로 저장된다 |
| `20260822170000_member_notes_tags` | **회원 메모·태그**(admin 회원 관리 2단계) — `member_notes`(회원당 여러 줄·시간순) + `member_tags`(회원당 여러 개·자유 입력 20자). RLS 는 `is_admin()` **하나뿐** | **실행 완료(2026-08-23 오너 실행 · anon 프로브 실측).** `GET /rest/v1/member_notes`·`member_tags` → **HTTP 200 `[]`**(표 있음 + anon 차단). 대조군으로 없는 표는 `HTTP 404 PGRST205` 가 온다 — 이 둘이 갈리는 것이 판정이다. ⚠️ **`authenticated` 용 select 정책을 만들지 말 것** — 회원 본인이 자기 상담 메모를 읽는다. ⚠️ 이 표의 **내용은 개인정보다**(값 insert 를 레포에 커밋 금지) |
| `20260824170000_spinning_day1_two_recordings` | **스피닝 첫날 녹음 2개**(2026-08-24 오너 "첫날에 녹음본 두 개") — `challenge_submissions.type` check 에 스피닝 한정 `before2` 추가 + `recordings` 버킷 쓰기/수정 정책의 파일명 패턴에 `spinning-before2.<ext>` 허용. mypage·admin 은 스피닝만 칸 셋(첫날 ①·② / 끝) | **적용 완료(2026-08-24 오너 실행 — 오너가 돌린 확인 쿼리 실측: `pg_constraint` 에 check 가 `before/after or (spinning and before2)` 한 줄(중복 없음 — UNIQUE(member_id,challenge,type) 줄은 원래 있던 upsert 대상), `pg_policies` 버킷 쓰기/수정 2건 모두 before2 포함. 다시 실행 안내 금지).** ⚠️ 이 마이그레이션은 **anon 프로브로 판별 불가**다(제약·정책이 전부 authenticated 뒤에서만 평가 — 익명 insert 는 제약 이전에 42501). 재확인이 필요하면 위 확인 쿼리를 다시 쓴다 |
| `20260824160000_drop_lesson_match_words` | **문항 판정 낱말 칸 제거**(2026-08-24 오너 "기능 없애버려 헷갈려") — `challenge_lessons.match_words` drop. 판정은 **문항 이름에서 만든다**(mypage `wordsOf()`) | **적용 완료(2026-08-24 오너 실행 — anon 프로브 실측: `select=match_words` 400 `42703` "column challenge_lessons.match_words does not exist", 화면이 쓰는 `day_no,title,summary` 조회는 200 10줄. 다시 실행 안내 금지).** 판정은 문항 이름에서 만든다 — 낱말 칸을 다시 만들지 말 것 |
| `20260824150000_blacklist` | **신청자 블랙리스트**(2026-08-24 오너 "우리만 알아볼 수 있게") — `blacklist`(phone 숫자만 unique · name · reason). RLS 는 `is_admin()` **하나뿐**(본인 포함 조회 창구 금지). admin 신청자 현황에 라벨·등록/해제 버튼 — 표시만, 신청·결제는 안 막는다 | **실행 완료(2026-08-24 오너 실행 · anon 프로브 실측).** `GET /rest/v1/blacklist` → **HTTP 200 `[]`**(표 있음 + anon 차단 — RLS 정상), 대조군 없는 표는 404. 다시 실행 안내 금지 |
...
- `save_my_profile`: `20260820170000` 을 재실행하면 `20260826150000` 의 본인인증 잠금
  (verified 회원 name·phone 보호)이 사라진다 — **최종 = `20260826150000`**.
...
- `20260806120000_credit_free_limit_fallback` — AI킬러 총량 무료 2회가 아직 나간다(위 표).
  **킬러를 내려 둔 동안은 무해하다** — 다시 열 때 같이 실행한다.
- `credit_free_limit()` 폴백에 `rehearsal → 1` 도 남아 있다. 리허설은 아직 출시 전이라
  지금은 무해하지만, 켤 때 같이 정리하지 않으면 같은 방식으로 새 나간다.

### ⏳ 2026-09-08 가격 개편 — 오너가 SQL 을 실행해야 반영된다

값은 `site_config` 라 **마이그레이션 파일이 아니라 SQL 한 번**이다(경위·근거는 credits.md
'2026-09-08 가격 개편'). 화면 코드는 이미 새 값을 전제로 폴백을 바꿔 두었다
(`credit-charge.js`·`mypage.html` 의 `COST_FALLBACK` = 소재 5 / 킬러 5 / 첨삭 15).

- `credit_costs` → `{"sojae": 5, "ai_killer": 5, "polish": 15, "rehearsal": 15}`
- `credit_daily_free` → `5`
- `credit_packs` → 3종(50/5,000 · 110/10,000 · 350/30,000, 전부 `tool:"ai_killer"`)
- 미실행 상태에서도 화면은 조용히 돈다 — 구 값(소재 1·하루 무료 10)이 그대로 보일 뿐이다.
