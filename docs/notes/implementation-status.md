...
| `20260805160000_member_course` | **승준노트 허브 코스** — `members.course`(beginner/practical/spurt/daily · null=미선택) + 체크 제약. 새 RLS 없음(기존 `members_update_own` 이 덮는다) | **실행 완료(2026-08-05 오너 실행·자기보고)** — ⚠️ **화면은 아직 배포 안 됐다**(아래 브랜치 절 참조 — main 은 구 매거진 목차형이다). 컬럼만 있고 쓰는 코드가 없는 상태라 무해하다. 미적용이면 허브가 '코스 선택 전' 화면만 보여 주고, [이 코스로 시작하기] 를 누르면 "지금은 코스 저장이 안 돼요" 를 그 자리에 띄운다(성공한 척 넘어가지 않는다). 도구 링크·서클·무료 맛보기는 그대로 동작. 조회는 `select('*')` 라 400 이 안 난다. 다른 표를 안 건드리므로 실행 순서 무관 |
...
- `claude/briefing-course-hub` — **승준노트 허브 코스형 개편(2026-08-05). 배포 보류 — 오너 지시 "브랜치에 넣어놓고 배포는 하지 마라".** 한 번 main 에 올렸다가(`7d0edec`) 곧바로 되돌렸다(`83fcb26`) — 라이브는 구 매거진 목차형이다. DB 컬럼(`members.course`)은 오너가 이미 실행해 놓았고, 쓰는 코드가 없어 무해하다.
  ...
  ⚠️ 남은 것: 코스 카드 사진 4장(오너 제공 대기 — `COURSES[].photo`), 코스별 시간 문구 검수.
