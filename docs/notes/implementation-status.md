...
| verify-identity | **레포 = `2026-08-26b`(⚠️ 오너 콘솔 배포 필요 — 미배포 상태)**: 실사고 "인증 확인에 실패했어요" 추적용 — ① 인증사 번호 +82 국제형·하이픈 정규화 ② 번호 누락(통합인증 창에서 카드·간편인증서 방식 선택 시)이면 bad_phone + verifiedCustomer 키 로그 ③ 성공 아닌 결과 전부 로그. 미배포여도 화면(온보딩 실패 코드 표시)은 따로 동작한다. **배포본 = `2026-08-26a` 배포 확인(2026-08-26 오너 배포 · anon 프로브 실측: `version` `2026-08-26a`). 같은 날 오너 실기기 인증 완주 확인("인증해봤는데 잘 되네") — 인증창→서버 검증→저장까지 전 구간 실전 검증 완료.** 휴대폰 본인인증(KG이니시스 통합인증 · 포트원 경유) — 온보딩이 보낸 `identityVerificationId` 를 포트원 API 로 재조회(`status==='VERIFIED'`)한 뒤 RPC `apply_identity_verification`(service_role 전용 · migration `20260826150000`)으로 실명·번호·CI·DI 저장 + CI·번호 중복 판정. 환경변수는 기존 `PORTONE_API_SECRET` 그대로(추가 설정 없음). 미배포여도 안전 — 온보딩이 프로브로 확인한 뒤에만 인증 UI 를 켠다(그전엔 직접 입력 폼) | `POST {"probe":true}` → `version` `2026-08-26a`. 404 면 미배포. 마이그레이션 미적용이면 본 호출이 `not_ready`(화면은 직접 입력 폴백) |
