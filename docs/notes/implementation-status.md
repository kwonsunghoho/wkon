...
| portone-webhook | **`2026-08-10a` 배포 확인(2026-08-10 오너 배포 · 키 없는 브라우저 프로브 실측으로 Verify JWT 꺼짐까지 확인).** 포트원 결제 통보를 받아 브라우저 없이 지급을 끝내는 최종 안전망(유실 0%). 남은 설정: **포트원 콘솔 > 웹훅에 URL 등록** — `https://apzwauiumhmsvrgffjis.supabase.co/functions/v1/portone-webhook` (V2). 등록 전까지는 통보가 안 와 기존 경로(브라우저 확인·자가 회복)만 돈다. 선택: 포트원 웹훅 시크릿을 Supabase Secrets `PORTONE_WEBHOOK_SECRET` 에 넣으면 서명까지 검증(없어도 안전 — 본문 불신·API 재조회). ⚠️ 결제 생성 때 실은 `customData.k` 가 있어야 지급한다 — 사이트 2026-08-10 배포 이후 결제부터 적용, 옛 결제는 기존 경로가 처리 | `POST {"probe":true}` → `version`·`has_secret` (**키 없이 호출해야 한다** — 401 이 오면 Verify JWT 가 다시 켜진 것). 실전 확인: 결제 하나 하고 포트원 콘솔 웹훅 발송 이력에서 200 응답 확인 |
