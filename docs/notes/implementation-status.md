...
| `20260805160000_member_course` | **승준노트 허브 코스** — `members.course`(beginner/practical/spurt/daily · null=미선택) + 체크 제약. 새 RLS 없음(기존 `members_update_own` 이 덮는다) | **실행 완료(2026-08-05 오너 실행·자기보고)** — ⚠️ **화면은 아직 배포 안 됐다**(아래 브랜치 절 참조 — main 은 구 매거진 목차형이다). 컬럼만 있고 쓰는 코드가 없는 상태라 무해하다 |
| `20260806090000_member_course_levels` | **코스 4단계 개편(초급~실전)** — 체크 제약을 `basic/mid/advanced/practical` 로 교체 + 구 값 이동(beginner→basic·daily→mid·spurt→advanced) | **owner 실행 필요(2026-08-06 작성)** — 코스형 화면(브랜치)이 새 코드값으로 저장한다. 미적용이면 [이 코스로 시작하기] 저장이 구 체크 제약에 걸려 실패하고 화면이 "지금은 코스 저장이 안 돼요" 를 그 자리에 띄운다(성공한 척 넘어가지 않는다). 도구 링크·서클·무료 맛보기는 그대로 동작. 조회는 `select('*')` 라 400 이 안 난다. **화면 배포 전에 실행해 두는 게 순서다**(컬럼 값만 바꿔서 라이브(구 화면)에는 무해 — 라이브는 course 를 안 읽는다) |
...
- `claude/briefing-course-hub` — **승준노트 허브 코스형 개편(2026-08-05). 배포 보류 — 오너 지시 "브랜치에 넣어놓고 배포는 하지 마라".** 한 번 main 에 올렸다가(`7d0edec`) 곧바로 되돌렸다(`83fcb26`) — 라이브는 구 매거진 목차형이다. DB 컬럼(`members.course`)은 오너가 이미 실행해 놓았고, 쓰는 코드가 없어 무해하다.
  ⚠️ **배포할 땐 이 브랜치를 main 에 병합한다.** 브랜치 끝 커밋이 revert 뒤에 있어 정상 병합된다. 브랜치를 `7d0edec` 로 되돌려 병합하면 git 이 '이미 합쳐졌다'고 보아 **아무것도 안 바뀐다**(revert 가 그대로 이긴다).
  ⚠️ 2026-08-06 브랜치에서 B안(세로 리스트·초급~실전 4단계·챌린지 추천 칸)으로 재개편 — 사진은 폐지(오너가 한글 칩 선택). 남은 것: **문구·스텝 확정(현재 가안 — 초급 서브만 오너 원문)**, 마이그레이션 `20260806090000` 오너 실행, 375px 실기기 확인.
