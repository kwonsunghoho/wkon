...
| `20260828120000_sojae_ke20_level` | **소재 발굴 KE20 섹션**(대한항공 대비 프로젝트 · 오너 지시 2026-08-28) — `questions.level` 체크 제약을 `basic/mid/advanced/deep/ke20` 으로 교체(drop→add · 재실행 안전). 문항은 오너가 admin '소재 문제'에서 직접 입력 | **owner 실행 필요(2026-08-28 작성).** 미적용이면 학생 화면 KE20 칸이 '0문제'로 뜰 뿐 그대로 돌고, admin 의 KE20 저장만 23514 로 막히며 화면이 이 파일명을 안내한다. 확인은 `pg_get_constraintdef`(20260806090000 줄과 같은 방법) 또는 admin 에서 KE20 문항 저장 성공 여부 |
