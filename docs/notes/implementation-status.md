...
| migration | 내용 | 기록된 상태 |
|---|---|---|
| `20260916120000_academy_student_requests` | **재학생 자가 신청 → 승인**(2026-09-16 오너 지시) — `academy_students` 에 `status(pending/approved)`·`member_id`·`requested_at`, `valid_until` nullable. `monc_is_student`·`my_student_status` 를 approved 기준으로 교체(+`status`·`verified` 반환), 신청 RPC `request_student_status`(인증 회원만) | **미적용 — 오너가 SQL Editor 에서 실행해야 켜진다.** 미적용 degrade: 마이페이지 신청 버튼이 `not_ready` 안내로 멈추고(칸은 status 없이는 안 그려짐), admin 은 대기 줄 없이 종전 목록. 프로브: anon 으로 `rpc/request_student_status` 에 `{}` → `42501`=적용, `PGRST202`=미적용 |
