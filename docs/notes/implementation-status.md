...
| ai-killer (polish·quickfix 겸용) | **배포본 = 레포 = `2026-08-12b` — 배포 완료(2026-08-13 anon 프로브 실측: `holistic`·`green_flags` 확인).** 2026-08-12 오너 지시로 킬러 판정을 규칙 엔진에서 **오너 지침 종합 판정(4기준 + AI 의심 지수 %)** 으로 전면 교체 + **점수 보정·그린 플래그** 지침 추가(첨삭·미니 다듬기는 그대로). 판정 피드백(만족/보통/불만족, 2026-08-13)은 **함수와 무관** — 브라우저가 `ai_killer_feedback` 에 직접 쓴다(재배포 불필요) | 로그인 없이 `POST {"probe":true}` → `version`·`features`·`airline_profiles`/`terms` 개수·`has_api_key`. `features` 에 `quickfix` + `quickfix_table` 숫자면 미니 다듬기 살아 있음. **`refund_server` 있으면 환급이 service_role 경로**(없으면 구버전 — 20260804160000 을 실행하면 안 된다). `coach_terms` 0 이면 연구진 자산 미유입. ⚠️ 오너 환경에선 크롬이 콘솔 붙여넣기를 막는다("allow pasting" 을 직접 타이핑해야 한다) — **콘솔 코드 화면에서 `FN_VERSION` 을 눈으로 보는 쪽이 빠르다** |
