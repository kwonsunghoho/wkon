...
| `20260802100000_signup_credit` | **가입 축하 크레딧** — `site_config.credit_signup_bonus`(15) + `grant_signup_credit()` RPC(회원당 1회·멱등) + 부분 유니크 인덱스 | **owner 실행 필요(2026-08-02 작성)** — 미적용이면 login 의 RPC 호출이 조용히 실패하고 지급만 안 된다(로그인·기존 크레딧 동작에는 영향 없음). 적용 확인: admin '크레딧' 탭 '가입 축하(1회)' 칸에 15 가 뜨고, 새로 로그인한 회원의 마이페이지 지갑에 15 가 찍히면 적용된 것 |
