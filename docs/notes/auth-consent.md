...
## 카카오 중복 가입 차단 — 2026-08-08 오너 지시(구글 버튼 잠금 후속)
...
- **막는 자리는 동의 게이트 하나뿐**: 새 계정이 만들어진 직후이자 가입 확정(동의) 전.
  `showConsentGate(session)` 이 ① 동의 기록 없음(isReturning=false) ② provider=kakao 일 때만
  `#dupBox`("혹시 예전에 구글로 가입하셨나요?")를 보인다. 동의가 끝난 뒤엔 늦다.
- '이 계정 만들지 않고 구글로 로그인하기'(`#dupSwitch`) → confirm → **기존
  `MONC.deleteMyAccount()` 재사용**(방금 만들어진 빈 계정이라 지울 것이 없다 — 두 벌 금지) →
  세션만 종료하고 로그인 뷰 복귀. ⚠️ `MONC.signOut()` 금지 — 홈으로 이동해 버려 구글로
  이어갈 수 없다. `MONC.sb.auth.signOut()` 으로 세션만 끝낸다.
- **인앱이면 복귀 시에도 구글 잠금 유지**(`btn.disabled = !!window.MONC_INAPP`), 안내는
  '브라우저에서 연 다음 구글 로그인'. `#inappNote` 에도 예방 문구 한 줄("카카오로 시작하면
  새 계정이 만들어져요")을 미리 둔다 — 게이트 도달 전에 알아채는 게 최선이다.
- 실패 방향은 '상자 없음': provider 판정이 안 되면 평소 게이트 그대로다. 게이트의 다른
  버튼 3개와 서로 disabled 를 걸어 이중 삭제를 막는다.
