...
- **복귀 인증 건은 세션 확인보다 먼저 보관한다(2026-08-26 저녁 학생 신고 "본인인증 이후
  로그인이 안 된다" 수리)**: 인증창(PASS)을 다녀오는 사이 세션이 끊긴 채 돌아오면
  `requireSession` 이 login 으로 보내는데, login 의 `safeReturnTo` 가 onboarding 을 제외해
  쿼리의 인증 건까지 유실됐다 — 재로그인해도 "또 인증하라"가 반복. 이제 온보딩이 로드 즉시
  (세션 확인 전) 인증 건을 sessionStorage `monc_idv_back` 에 보관하고, 재로그인 뒤 재방문 때
  이어서 확정한다. 서버가 답(성공·실패 코드)을 주면 보관을 지우고, 네트워크 실패만 남겨 다음
  방문에 재확정한다(같은 계정 재확정은 RPC 멱등 통과 — `already:true`). 인앱→외부 브라우저처럼
  탭 자체가 바뀌면 보관이 못 넘어간다(그 경우는 재인증이 답). 같은 수리에서 **인증창 복귀
  주소에 `verify=1` 을 동반**하게 고쳤다 — 빠지면 복귀가 가입 온보딩 모드로 돌아 not_ready
  폴백이 기존 회원에게 직접 입력 폼을 보여 주고 통과 티켓(`monc_idv_pass`)도 못 끊었다.
- **인앱 브라우저(인스타·카톡)는 인증을 강요하지 않는다(2026-08-27 신고 "인스타 유입이 로그인
  본인인증 화면에서 멈춘다" 수리 · 같은 날 오너 확정 "이대로 가자" — 영구 면제가 아니라 유예:
  같은 학생이 일반 브라우저로 오면 게이트가 다시 요구하고, 직접 입력 폼도 서버 번호 대조는
  그대로 탄다는 설명 위에서 확정)**: 인앱 웹뷰는 인증창의 PASS·간편인증 앱 전환이 막혀 완주가
  안 된다 — 신고 시점 함수 Logs 실측도 실패 기록 0(막힌 사람은 서버까지 도달 못 함). 처리는
  '지금 인증 불가' 환경과 동일한 fail-open: ① `requireVerified()` 가 `MONC_INAPP` 이면 통과
  (④번째 안전장치) ② 온보딩 가입 모드는 인증 모드를 안 켜고 직접 입력 폼 폴백(인앱 판정이
  defer 로 늦는 레이스는 버튼 클릭 시점 방어가 한 번 더 잡는다) ③ 인증 전용 모드(?verify=1)는
  통과 티켓(`monc_idv_pass`) ④ 마이페이지 자발 인증 버튼은 login 구글 버튼과 같은 외부 브라우저
  우회(안드로이드 크롬 인텐트 / 아이폰 여는 법 alert). 이날 onboarding 에 inapp.js 합류(배너
  포함 — page-common.md 제외 목록도 갱신). 인증창 복귀 확정(backId)은 인앱이어도 그대로 처리한다.
  일반 브라우저에서는 그대로 인증을 요구한다 — 인앱 통과를 넓히거나 좁히려면 오너 확인 먼저.
- **확정 실패 문구는 실패 코드를 같이 보여 준다(2026-08-26 실사고 — 학생 캡처가 "인증 확인에
  실패했어요"뿐이라 원인을 못 좁혔다)**: invoke 예외는 HTTP 상태·본문 code 를, 서버 200 실패는
  `code · message` 를 괄호로 붙인다. `bad_phone`(통합인증 창에서 카드·간편인증서 방식을 고르면
  인증사가 번호를 안 줄 수 있다)은 "휴대폰(통신사) 인증으로 진행" 전용 안내. 서버(verify-identity
  `2026-08-26b`)도 +82 정규화 + 비정상 결과 로그를 남긴다 — 원인 추적은 함수 Logs 가 원장.
