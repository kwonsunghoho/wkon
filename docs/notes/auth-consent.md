...
## 인앱 브라우저에서 구글 버튼 잠금 — 2026-08-08 오너 지시

인스타·카톡 등 인앱 웹뷰는 **구글이 로그인 자체를 정책으로 거부한다**(disallowed_useragent).
버튼을 그대로 두면 눌렀다가 구글 쪽 오류 화면에서 끝난다 — 우리 화면이 아니라 안내도 못 한다.
오너가 "없애버리던가"라고 했지만 **전면 삭제는 기각**: 구글로 가입한 기존 회원이 로그인 길을
잃는다(카카오로 다시 로그인하면 기록·크레딧 없는 새 계정이 생긴다).

- **동작**: 인앱이면 login.html 이 ① 카카오 버튼을 첫 번째로 올리고 ② 구글 버튼을 disabled +
  바로 아래 `#inappNote` 로 이유("앱 안 브라우저에서는 구글 로그인이 막혀 있어요") + 대안
  (카카오 / 상단 배너로 외부 브라우저) ③ `.sub` 문구도 '카카오 계정으로'로 교체.
  일반 브라우저는 아무것도 안 바뀐다.
- **⚠️ 판정은 `inapp.js` 의 `window.MONC_INAPP` 단일 소스** — UA 목록을 login 에 복사하지 말 것.
  플래그는 배너 닫힘(sessionStorage)과 무관하게 early return **앞에서** 내보낸다.
- **⚠️ inapp.js 가 `defer` 라 플래그는 DOMContentLoaded 에야 있다** — login 의 인앱 분기가
  그 리스너 안에 있는 이유. 앞으로 당기면 항상 undefined 라 잠금이 조용히 죽는다.
- **⚠️ OAuth 실패 복구 두 곳이 `btn.disabled = !!window.MONC_INAPP`** — `false` 로 되돌리면
  카카오 로그인이 한 번 실패하는 순간 잠가 둔 구글 버튼이 되살아난다(실측으로 잡음).
- **실패 방향은 '평소 화면'**: 플래그가 없으면(inapp.js 구버전 캐시·로드 실패) 아무것도 안 바꾼다.
  구글 버튼을 숨기는 방식으로 바꾸지 말 것 — 기존 구글 회원에게는 "내 로그인이 없어졌다"다.
- 검증: `scratchpad/login-inapp.mjs`(인스타 UA ↔ 일반 UA · 카카오 실패 복구 후 잠금 유지 · 375px).
- **미검증 하나**: 인스타 웹뷰에서 **카카오 OAuth 완주**는 실측 전이다(막힌다는 기록은 없음 —
  카카오는 구글 같은 웹뷰 차단 정책이 없다). 오너 폰 실측 대기.
