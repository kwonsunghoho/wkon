...
## 결제 버튼 아래 카카오톡 문의 한 줄 (2026-08-09 오너 확정)
...
- **색은 카카오 노랑 `#FEE500` + 먹색 `#191919`**(오너 "버튼은 카카오색으로") — 브랜드 기능색이라 네이비 팔레트 통일 대상이 아니다. 결제 버튼(토스 파랑·네이비)보다 눈에 덜 띄는 보조 줄로 유지 — **결제 CTA 보다 크게 만들지 말 것.**
...
localStorage 가 원장이고, 그래도 유실된 건은 admin 이 포트원 내역으로 살린다.
