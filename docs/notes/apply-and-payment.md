...
- 지웠던 표현 — **되살리지 말 것**: `24시간 이내 1:1 피드백` · `코치 개별 피드백` · `매일 미션을 올리면 코치가 다음 날 코멘트를 남깁니다` · `매일 하나씩 쓰고 피드백받으며`.
- 고친 곳 12군데: `apply.html`(히어로 배지·상품정보 3줄·FAQ) · **`terms.html`(상품형태·제공방식·진행주기)** · `login.html` 혜택 줄 · `mypage.html` 상태 문구 2곳 · `challenge-answer.html` 추천 카드.
