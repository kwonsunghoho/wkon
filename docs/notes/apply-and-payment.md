...
`docs/superpowers/specs/2026-09-09-academy-student-free-design.md`, 명단 화면은 admin.md '재학생 명단'.

- **판정은 서버 하나** — `monc_is_student()`(migration `20260909120000`):
  **본인인증(`members.verified_at`)한 번호**가 `academy_students` 에 있고 종료일(`valid_until`)이
  안 지났으면 재학생. ⚠️ 인증을 요구하는 것이 문지기다 — 프로필 번호는 '다른 계정과 겹치는가'만
  보므로 아무도 안 쓴 남의 번호를 적을 수 있다.
- **명단 줄에는 상태가 있다(2026-09-16)**: `pending`(학생이 마이페이지에서 신청 · 승인 대기) / `approved`(무료).
  판정은 `approved` + `valid_until` 있음 + 기간 안만. 신청·승인 흐름은 admin.md '승인 대기'.
- **접수는 `apply_free_challenges(p_ids text[])`** — 브라우저는 **고른 챌린지 id 만** 보낸다.
  기수는 서버가 `challenge_rounds` 에서 '모집 중'인 것을 다시 읽고, 금액은 0 고정, 신청자
  이름·전화는 `members` 에서 읽는다. ⚠️ 기수를 브라우저가 정하게 하면 마감된 기수에 무료로 끼어든다.
- **중복 신청·정원 판정은 기존 트리거 그대로다**(MC002 → `{ok:false, code:'duplicate', program}`).
  `payment_status='free'` 는 `monc_app_live()` 가 '살아있는 신청'으로 세므로 판정이 저절로 맞는다.
- **원장은 여전히 `applications.payment_status='free'` 하나.** 자동(RPC)·수동(admin 토글)을
  구분하는 컬럼을 만들지 말 것 — 미입금 판정 두 곳과 CSV '재학생' 칸이 이 값 하나를 본다.
- **화면(`apply.html`)** — 카드 금액·요약·하단바가 0원, 결제 버튼 묶음(`#tossWrap`·`#submitBtn`)과
  환불 고지를 숨기고 `#freeWrap` 의 [무료로 신청하기] 하나. 동의 체크는 그대로 필수.
  `initStudent()` 가 이미 그려진 카드의 `.card-price` 도 갈아끼운다 — **`renderCards()` 를 다시
  부르지 말 것**(클릭 리스너가 날아가 카드가 안 골라진다).
- **비재학생 화면은 한 줄도 바뀌지 않는다.** 공개 페이지에 '재학생 무료' 문구를 걸지 않는다는
  2026-08-11 원칙 그대로 — 걸면 누구나 무료를 주장하고 PG 심사에도 불리하다.
- ⚠️ **판정 조회 실패는 '재학생 아님'(유료)이다** — `MONC.studentStatus()` 는 본인인증 게이트
  (`requireVerified`, fail-open)와 **실패 방향이 반대**다. 무료가 장애로 열려선 안 된다.
- 크레딧 충전은 대상이 아니다(AI 원가가 매번 나간다 — 오너 확정 2026-09-09).
...
- **`program_start` 는 선착순에서도 저장한다**(2026-09-03) — 학생 화면(마이페이지·submit)의 문항·제출 칸이
  열리는 날. 규칙(`round-gate.js roundStartOf`)과 배경은 mypage.md '시작 전 잠금'.
