...
- **⚠️ 잔여석 자동 카운팅(2026-07-24 신설 · migration `20260724150000_lecture_seat_counting.sql`, owner 실행 필요):** **`capacity`(정원)가 기준값, `seats_left`(잔여석)는 파생 캐시**다. `applications` 트리거가 신청이 들어올 때마다 `잔여석 = 정원 − 신청건수`로 **재계산**한다(차감 −1이 아님 — 값이 어긋나도 진실로 복구되고, admin이 신청을 지우면 자리가 자동으로 돌아온다). **정원이 NULL인 특강은 트리거가 손대지 않는다**(구 수동 운영 유지).
  ...
  - **⚠️ 잔여석 '보이기/숨기기' 스위치(2026-08-19 오너 요청 · migration `20260819130000_lecture_show_seats.sql`, owner 실행 필요):** `special_lectures.show_seats`(기본 true). 신청이 저조하면 잔여석이 안 줄어 그 숫자가 그대로 노출되는 게 문제였다. admin '특강' 탭의 **[신청 페이지에 잔여석 보이기]** 체크를 끄면 카드 메타줄·상세 fact·시간대 목록에서 **숫자만** 사라진다.
    - **표시 스위치일 뿐이다 — 판정은 그대로 DB 가 한다.** 잔여석 계산 트리거·정원 초과 차단(MC001)·`ensureSeatOpen()` 은 숨김과 무관하게 돈다. 숨겨도 **'정원 마감'·'신청 마감'은 계속 말한다**(못 신청하는 이유를 안 알려주면 안 된다 — 카드 흐림·버튼 비활성도 유지). 상세 fact 는 잔여석 대신 **정원 N명**을 쓴다.
