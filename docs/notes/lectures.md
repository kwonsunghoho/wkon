...
유료 특강도 재학생 계정에서는 0원이다. 판정·명단은 apply-and-payment.md '재학생 무료 참여' 참조.

- `applyBoxHtml()` 의 `stuFree = !free && _isStudent` 한 갈래 — 참가비 줄이 '무료',
  결제 버튼 3종 대신 `#stuFreeBtn` [무료로 신청하기] + 안내 한 줄(`.lc-stufree`).
- 접수는 `apply_free_lecture(p_lecture, p_slot)`. **시간대가 이 특강의 것인지 서버가 다시 본다**
  (verify-payment 와 같은 검사). 좌석은 기존 트리거가 센다 — `payment_status='free'` 도
  `monc_app_live()` 가 '살아있는 자리'로 보므로 정원이 정확히 한 칸 줄어든다.
- `_isStudent` 는 `load()` 에서 **첫 렌더 전에** 읽는다 — 나중에 알면 결제 버튼이 잠깐 보였다 바뀐다.
- `not_student` 응답이 오면 `_isStudent=false` 로 되돌리고 화면을 다시 그린다(무료 버튼만 남으면 계속 막힌다).
...
- **카드 머리 = 특강 시작일 큰 날짜**(2026-08-26 오너 확정 "시작일이 제일 크게" — 목업 `outputs/lecture-date-mockup.html` A안): `.lc-dhead` 라벨 12px + 날짜 32px·800 먹색 + 꼬리(요일·시간·장소) 17px·700 네이비. 구 '일시' 16px 줄을 대체한다 — 카드 안에서 참가비(20px)보다 큰 유일한 값. **날짜를 못 읽는 특강(추후 안내)만 종전 '일시' 줄로 폴백**, 타임 여럿이면 첫 날짜 크게 + 'N개 타임' 요약(자세한 건 아래 시간대 목록).
...
- **⚠️ 잔여석 자동 카운팅(2026-07-24 신설 · migration `20260724150000_lecture_seat_counting.sql`, owner 실행 필요):** **`capacity`(정원)가 기준값, `seats_left`(잔여석)는 파생 캐시**다. `applications` 트리거가 신청이 들어올 때마다 `잔여석 = 정원 − 신청건수`로 **재계산**한다(차감 −1이 아님 — 값이 어긋나도 진실로 복구되고, admin이 신청을 지우면 자리가 자동으로 돌아온다). **정원이 NULL인 특강은 트리거가 손대지 않는다**(구 수동 운영 유지).
  ...
  - **⚠️ 금액 이름 참가비/예약금(2026-09-12 오너 요청 · migration `20260912120000_lecture_price_kind.sql`):** `special_lectures.price_kind`(`fee` 기본 | `deposit`). admin 폼 '금액 이름' 셀렉트에서 예약금을 고르면 **카드 `.lx-price` 라벨·상세 정보 카드 라벨·신청 폼 `.pay-total` 라벨이 '예약금'**이 되고, 상세 정보 카드와 신청 폼 금액 아래에 **'잔금은 별도로 안내드려요.' 한 줄**(`LEC.DEPOSIT_NOTE` · `.lc-deposit-note`)이 붙는다. 하단 고정바·admin 목록은 라벨 자리가 없어 금액 앞에 '예약금 ' 을 붙인다.
    - **판정은 `LEC.isDeposit(l)` · 라벨은 `LEC.priceLabel(l)` 한 곳** — `price > 0 && price_kind === 'deposit'` 일 때만 예약금(0원 특강은 '무료'/문구가 나가므로 라벨을 안 바꾼다). 페이지마다 `'참가비'` 문자열을 직접 쓰지 말 것. 마이그레이션 미적용이면 컬럼이 없어 '참가비'로 동작하고, admin 저장은 `LEC_OPT_COLS` 재시도로 그 컬럼만 빼고 저장된다.
    - **라벨만 바꾼다** — 청구·verify-payment 검증 금액은 `price` 그대로다. 잔금은 사이트가 받지 않는다(상담·현장). 환불 안내문('특강 시작 전 취소는 전액 환불')은 예약금이어도 그대로 둔다 — PG 심사에서 거절된 '보증금·환급' 워딩을 예약금 쪽에 끌어오지 말 것(apply-and-payment.md).
