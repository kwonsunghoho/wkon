...
> **⚠️ 2026-07-22 스펙 추가 결정 — 이 계획의 코드에 아직 미반영. 구현 시 스펙(§2 07-22 행, §3 tag, §5)을 우선하라:**
> 1. **제목 검색창**(전체 뉴스 탭, `ilike` + 디바운스 300ms) → Task 4에 추가.
> 2. **스크랩 자유 태그**: `news_scraps.tag text null`(Task 1 마이그레이션에 컬럼 추가) + 내 스크랩 탭 태그 지정 UI·태그 필터칩(Task 5에 추가). 추천 예시 칩 + 내가 쓴 태그 재노출. 저사용 시 제거 가능한 실험 기능 — 내 스크랩 탭 밖으로 의존 금지.
> 3. **스크랩 버튼 = 몬크 로고 창문 심볼**(리본 북마크 금지): 인라인 SVG 1벌, 전 상태 `grayscale` 필터·후 상태 원색(Task 4·5의 북마크 버튼 마크업 대체).
