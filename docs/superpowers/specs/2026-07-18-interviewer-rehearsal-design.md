...
1. **면접관 리허설 (이 스펙)** — 완료한 답변을 놓고 연구원이 면접관 질문 패턴으로
   **대화형 코칭**(꼬리질문 → 회원 답변 → 즉석 코칭 반복 → 종합 첨삭). 오너 방향 수정
   2026-07-18: "꼬리질문 검증"이 아니라 **"답변 작성법을 가르치고 첨삭해주는"** 성격이 핵심
...
3. **코칭 세션** — 연구원이 면접관 질문 패턴으로 꼬리질문을 던지고, 회원이 답하면
   **즉석 코칭**(잘한 점 한 줄 + 보완 한 가지)을 주고 다음 질문으로. **3~4라운드,
   총 7~9왕복.** 압박이 아니라 과외 — 회원은 라운드마다 배우고 나아진다.
   대화는 저장되어 중단 후 이어하기 가능.
4. **종합 첨삭 리포트** — 마지막 라운드 뒤: 강점 / 보완점 / **문장 단위 개선 방향**
   ("마무리를 '배움→기내 실천'으로 연결") / 대비해둘 예상 꼬리질문.
   "답변 보완하러 가기" → `sojae.html?q=<question_id>`.
   **⚠️ 대필 금지 경계:** 문장 단위 개선 *방향*까지만 — 통째로 고쳐 쓴 완성 답변은
   주지 않는다(소재 발굴의 "답을 대신 만들어주지 않는다" 철학과 동일. 외운 티 나는
   답변 방지 + "자기 말로 완성한 답변"이라는 상품 가치 보호).
...
- 프롬프트: 시스템 = 코칭 지침 + **유형별 꼬리질문 패턴집 + 연구진 첨삭 기준**(아래) 주입.
  화자는 기존 소재 발굴과 동일한 "연구원" 페르소나(일관성) — 면접관 질문 패턴을
  인용하며 코칭한다. 모델은 답변 스냅샷·대화 이력을 보고 [꼬리질문 1개] 또는
  [직전 회원 답변에 대한 즉석 코칭 + 다음 질문]을 낸다. 3~4라운드 뒤
  종합 첨삭 리포트를 내고 종료 신호 마커를 붙인다 →
  클라이언트가 verdict(첨삭 리포트) 저장 + status='done'.
  대필 금지 규칙을 시스템 프롬프트에 명시(완성 답변 생성 금지).
...
- `site_config` key `rehearsal_patterns` (JSON: category → 꼬리질문 패턴 목록 +
  **연구진 첨삭 기준**(무엇을 강점/보완으로 보는가 — 3,500명을 가르친 평가 관점)).
