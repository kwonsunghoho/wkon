...
- 날짜: 2026-07-21 (2026-07-22 오너 추가 결정 반영: 검색창·스크랩 태그·로고 스크랩 버튼)
...
| 검색 (07-22) | 전체 뉴스 탭에 제목 검색창 추가 — 자동 분류 세분화 대신 검색으로 해결(오너 "산업에 대해 세분화" 요구의 채택안; 분류를 잘게 쪼개는 안은 키워드 오분류 증가로 기각) |
| 스크랩 태그 (07-22) | **자유 입력** 단일 태그 + 추천 예시 칩 — 내 스크랩만의 세분화 축. "사용 안 하면 기능을 없애도 된다"(오너)는 가벼운 실험 포지션 |
| 스크랩 버튼 (07-22) | **몬크 로고 창문 심볼 그 자체** — 스크랩 전 무채색(grayscale)·저채도, 스크랩 시 원색 로고 완성("이 풍경, 내 창에 담았다"). 리본 북마크 금지 |
...
- `tag` text null — **자유 입력 태그**(2026-07-22 추가; "지원동기 재료", "산업 트렌드" 등 회원이 직접 붙이는 분류. 단일 태그 — 다중 태그는 사용 패턴 확인 후)
- `created_at` timestamptz default now()
- **unique(member_id, article_id)**
- RLS: 본인 것만 SELECT/INSERT/UPDATE/DELETE (`member_id = auth.uid()`). UPDATE는 메모·태그 수정용.
...
3. 필터 칩 2줄(reviews.html 패턴): 항공사(전체 + 데이터 존재값만), 주제(전체 + 데이터 존재값만). 두 축 AND 결합.
   - **검색창**(2026-07-22 추가): 제목 부분일치(`ilike '%q%'`), 필터 칩과 AND 결합. 디바운스
     300ms, 빈 입력이면 검색 미적용. 자동 분류를 잘게 쪼개는 대신 세분 탐색은 검색이 담당.
4. 기사 카드 리스트(최신순): 제목(2줄 클램프) · 언론사 · 상대시각("3시간 전") · 항공사 칩 · 주제 칩 · **스크랩 토글 버튼**(우상단, 44px 터치 타겟).
   - **스크랩 버튼 = 몬크 로고 창문 심볼**(`images/몬크 로고/assets/MONC-symbol.svg` 기반
     인라인 SVG 1벌): 스크랩 전 `filter:grayscale(1)`+저투명(색 빠진 창), 스크랩 시 원색
     로고 완성(남색 창틀+노을 그라데이션). 그림 두 벌 만들지 말고 CSS 필터로 상태 전환.
     리본 북마크 아이콘 사용 금지(오너 확정).
   - 카드 본체 클릭 → 원문 새탭(`rel="noopener"`).
   - 스크랩: 회원이면 즉시 insert/delete + 아이콘 토글(낙관적 업데이트, 실패 시 롤백). 비회원이면 안내 후 login.html 이동.
...
6. '내 스크랩' 탭: `news_scraps` join `news_articles`. **항공사별 그룹핑 우선** —
   ...
   - 카드마다 **활용 메모** 표시·수정: 메모 없으면 "메모 남기기" 버튼, 있으면 메모 텍스트
     ...
     않음(스크랩 한 번 누르는 흐름을 무겁게 하지 않기 위해 — 메모는 내 스크랩에서 추가).
   - **태그**(2026-07-22 추가, 내 스크랩 탭 전용 세분화 축):
     - 카드마다 태그 표시·지정: 태그 없으면 "태그 붙이기" 버튼 → 자유 입력 + **추천 예시 칩**
       (지원동기 재료 · 최근 이슈 · 산업 동향 · 서비스 아이디어 — 탭하면 그대로 입력됨,
       내가 이미 쓴 태그들도 예시로 노출해 표기 흔들림 방지). 메모와 동일하게 스크랩
       시점엔 강요하지 않음.
     - 탭 상단에 **내 태그 필터 칩**(내 스크랩에 존재하는 태그만 동적 생성, reviews.html
       패턴). 항공사 그룹핑과 AND 결합 — "대한항공 × 지원동기 재료" 조합이 핵심 쓰임새.
     - 태그가 하나도 없으면 태그 필터줄 자체를 숨김(기능이 비어 보이지 않게). **사용률이
       낮으면 기능째 제거 가능**한 구조로 — 태그 UI는 내 스크랩 탭 안에만 두고 다른
       화면(mypage 카드·전체 뉴스 탭)에 의존시키지 않는다.
...
## 7. 내비게이션 — ⚠️ 2026-07-22 '브리핑룸' 허브로 개편(오너 결정)

nav에 '뉴스'를 직접 넣지 않는다. **`briefing.html`(브리핑룸) 허브**를 신설하고 nav는 '브리핑룸' 하나만:

- **브리핑룸 = 면접 준비 도구 종합세트.** 카드: 항공 뉴스(누구나, 로고 창문 아이콘) → news.html /
  소재 발굴(수강생 전용) → sojae.html / 답변노트(회원 전용) → mypage.html#sec-answers /
  실전 모의면접(배포 보류 — 마크업만 두고 `display:none`, 열 때 제거).
- 이름 '브리핑룸'은 승무원 비행 전 브리핑에서 — 직관+친근+브랜드(오너 확정, 라운지·활주로 기각).
- 도구가 늘면 nav는 안 건드리고 허브에 카드만 추가한다.
- nav 실제 위치: index.html·researchers.html(풀 메뉴 보유 페이지 둘뿐)의 데스크톱+모바일 메뉴.
  나머지 서브페이지는 로고+'← 홈으로' 패턴이라 해당 없음. news.html 뒤로가기는 '← 브리핑룸'.

## 8. 비범위 (YAGNI)

- 댓글·좋아요·조회수 없음.
- 기사 본문 저장 없음(제목+링크만 — 저작권·저장 비용 회피, 원문은 언론사에서).
- ~~검색창 없음~~ → **2026-07-22 범위 편입**(제목 검색, §5). 단 본문 검색·고급 검색은 없음.
- 주제 자동 분류 세분화 없음 — 키워드 매칭 한계로 오분류만 늘어 기각(세분 탐색은 검색창, 개인 세분화는 스크랩 태그가 담당).
- 스크랩 다중 태그 없음(단일 태그로 시작, 사용 패턴 확인 후).
