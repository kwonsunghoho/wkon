...
- RLS: 본인 것만 SELECT/INSERT/DELETE (`member_id = auth.uid()`). UPDATE 불필요.
...
4. Supabase REST API로 upsert — `url` unique 충돌 시 **무시**(`Prefer: resolution=ignore-duplicates`).
...
6. '내 스크랩' 탭: `news_scraps` join `news_articles`, 동일 카드·필터, 북마크 해제 시 목록에서 제거.
