...
## 2차 확장(2026-08-26 오너 지시 — 같은 날)

"1번(마이페이지 자발 인증 버튼)으로 만들되, 본인인증한 사람만 특강·승준도구·연구실 사용."

- `MONC.requireVerified()` 게이트: 승준 도구 진입 6곳 + 특강 신청 단추 3곳 + 연구실 자료
  열기·구매 2곳. 미인증 → `onboarding.html?verify=1`(인증 전용 모드) → 인증 후 복귀.
- fail-open 3중(조회 실패·프로브 실패 캐시·온보딩 통과 티켓) — 인프라 장애가 잠금이 되지 않게.
- 마이페이지 미인증 회원에게 '휴대폰 본인인증으로 확인하기' 버튼(`#phoneVerify`).
- 부르는 곳 원장은 auth-consent.md '휴대폰 본인인증' 절.

## 하지 않는 것(이번 범위 밖)

- admin 인증 여부 칸 · 챌린지 신청(apply) 게이트 · 게임·뉴스 게이트 · 함수 서버측 verified
  강제(우회가 관찰되면 그때) · 웹훅(돈이 아니라서 유실 시 재인증으로 충분).
