...
- `polish.html` — **첨삭(2026-07-30 신설).** 진단(AI킬러)의 짝인 **처방** — 강점(지킬 것)·글 전체 방향·문장 단위 '지금 → 이렇게' 리포트. 유료 도구 2개(소재·킬러)가 둘 다 진단이라 **진단 직후 지갑이 열리는 순간에 팔 처방이 없던 구멍**을 메운 것(오너 확정 — 리허설은 이 위의 상위 상품으로 보류, 자산은 `claude/rehearsal-wip`). 진입 2곳: 킬러 결과 dock `.dock-cta`('이 글 첨삭받기 · 첫 1회 무료/10크레딧') + answers.html 카드 이력·CTA. 입력부는 ai-killer.html 과 같은 문법(종류·항공사·문항·바텀시트·충전), 결과부는 dock 순회가 아니라 **통독하는 리포트**(그래서 dock 없음 — 액션은 리포트 끝).
  ...
  - **⚠️ 서버 p-1: `answer_polishes` count 실패(마이그레이션 미적용) 시 반드시 차감 전에 `not_ready`.** 무시하고 진행하면 차감 키(`<answer_id>#p<이전횟수>`)의 횟수가 늘 0 → 두 번째부터 spend_credit 이 'already' 통과 — **첨삭이 영영 공짜가 되는 돈 버그**다.
  - **⚠️ 재검사 무차감 없음** — 킬러의 MAX_RECHECK 와 다른 점. 첨삭은 매 회 차감(무료는 가입 후 총 1회 — `credit_free_limits.polish`, DB 적용 완료). 고친 뒤 확인은 킬러(3)가 담당하는 설계 동선: **첨삭 → 고침 → 'KILL AI로 확인'**(결과 화면 링크가 같은 답변을 물고 넘어간다).
  - **⚠️ fix 는 학생이 쓴 사실만 재료로**(프롬프트 '첨삭의 선'). 없는 숫자·일화를 지어내면 학생이 그 거짓을 면접장까지 들고 간다 — 부족한 자리는 **(괄호 빈칸)**으로 남기고 화면이 그 괄호를 칠해 준다(`markBlanks`). 합격자 문장 흉내·옮겨쓰기 금지는 킬러와 동일(결정 10).
...
- **크레딧 차등**: 소재 발굴 **2** / KILL AI **3** / 첨삭 **10**. ⚠️ **가격은 원가가 아니라 가치를 따랐다** — 첨삭 원가는 킬러의 두 배쯤인데 단가는 세 배다. 학생이 돈을 내는 이유가 진단이 아니라 처방이라서다. **진단이 싸야 자주 쓰고, 자주 써야 처방이 팔린다.** 원가 비율로 되돌리지 말 것. **첨삭은 2026-07-30 실제 출시**(그 전까지는 가격표·admin 칸만 있고 코드가 없는 유령 상품이었다 — 아래 `polish.html` 항목 참조).
...
- **⚠️ 소재 발굴도 2026-07-27부터 크레딧으로 통제한다 — 구 `members.sojae_enabled` 권한 스위치는 폐지.** 회원이면 누구나 들어가고, 값은 **다듬기 한 번에만** 붙는다(되묻기 대화는 무료). 관련 UI(admin 회원별 토글·일괄 켜기/끄기·목록 '소재' 배지, mypage 카드 잠금, sojae 권한 차단 화면, `MONC.hasSojaeAccess`)는 전부 삭제 — **권한 플래그 방식으로 되돌리지 말 것.** 서버(`sojae-chat`)의 권한 검사도 함께 제거됐다.
  ...
  - **⚠️ 리허설(모의면접) 단가는 3→15 로 선반영(2026-07-30 오너 확정 · migration `20260730130000`).** 미출시 상품이라 지금은 무해하지만, 구 값 3은 하루 무료 5 아래라 **출시하는 날부터 매일 공짜**가 되는 지뢰였다(첨삭 10 잠금과 같은 원리). 출시 때 값을 다시 정하더라도 하루 무료보다 반드시 크게 — admin '크레딧' 탭의 경고가 이 조합을 막는다.
...
- Active detail pages (challenges.html 카드에서 링크, 신청은 `apply.html?c=<id>`로): `challenge-voice.html`(보신각), `challenge-expression.html`(영합각), `challenge-spinning.html`(스피닝), `challenge-answer.html`(승자각). ⚠️ 블라인드 퀴즈는 2026-07-30 허브(challenges.html) 하단으로 이사 — 상세엔 없다.
  ...
  - **2026-07-30 확정 팔레트 적용 완료(오너 "가독성 자체가 말이 안 된다 / 홈 색에 맞춰라").** 되돌리지 말 것: ① CTA = `--action` 네이비 면 + 흰 글씨(구 갈색→네이비 그라디언트) ② 마무리 `.cta-box` = `--accent`→`--accent-dark` 네이비 무대 + **흰 알약 + 네이비 글씨** 버튼 ③ 챌린저 아바타 원은 **전부 같은 네이비 단색**(구 6종 갈색·오렌지 그라디언트 — 그중 `#FF6B35→#FFB088` 은 흰 글씨 대비가 2:1 대였다) ④ BEFORE = 종이색 / AFTER = `--action-tint` ⑤ 섹션 배경 워시(구 오렌지·갈색 반투명 그라디언트) → `--bg2` 단색.
...
- **② 열다섯 가지 → 세 가지**("혼자하면 막막하지만, 몬크와 함께하면 **간결합니다**." — 2026-07-30 오너 문구): 칩 **15개**(자소서(FSC·LCC) · 답변(차별화·소재발굴) · 보이스(목소리) · 스피치(말 맛) · 이미지 연출(헤어&메이크업) · 기업정보(단순 정보) · 기업분석(기업문화 이해) · 카메라연출(구도·조명) · 필수기출 · 롤플레잉 · 영어인터뷰 · 멘탈관리 · 비언어 표현 · 스펙관리 · 이력관리 — **목록·괄호 설명·나열 순서 전부 오너 확정**, 같은 날 10개에서 확장)가 하나씩 체크된 뒤, **같은 칩이 제자리에서 세 그룹으로 날아가 붙는다**(FLIP). `data-g` 1=승준노트 2=챌린지 3=특강 → **③의 문 순서와 같다**. 그룹 구성(오너 확정): **승준노트** 자소서·기업정보·필수기출·답변 / **챌린지** 보이스·스피치·이미지 연출·비언어 표현·카메라연출·롤플레잉 / **특강** 기업분석·영어인터뷰·멘탈관리·스펙관리·이력관리. ⚠️ **`data-o` 는 그룹 안에서의 자리**다 — 나열 순서와 그룹 순서가 서로 달라서 따로 들고 있어야 한다(없으면 그룹 안 순서가 나열 순서를 따라간다). ⚠️ **높이 재기(`reserve`)와 실제 이동(`place`)이 같은 순서(`toGroups()`)를 써야 한다** — 부제가 있는 칩과 없는 칩의 높이가 달라, 순서가 다르면 줄마다 높이가 달라져 미리 잰 값이 어긋난다. ⚠️ 부제(`.sc-t em`)는 **12px 하한**이고 `.sc-t` 의 `line-height:1.35` 가 핵심이다 — body 의 1.7 을 물려받으면 두 줄 칩이 64px 까지 부풀어 목록이 화면 하나를 통째로 먹는다(실측). ⚠️ 칩 나열 순서는 일부러 섞여 있다(그룹 순으로 늘어놓으면 '정리되는 순간'이 안 보인다). ⚠️ 그룹 제목은 문 이름과 **글자 그대로 같아야** 방금 정리한 세 덩어리가 아래 세 문과 같은 것으로 읽힌다. ⚠️ **한 번만 실행**(`io.disconnect`) — 되감기를 넣으면 스크롤을 오르내릴 때마다 글이 다시 움직인다. ⚠️ **전체 1.5초 한도(오너 확정)는 칩 개수와 한 쌍이다** — 10개 시절 값(STEP 70·HOLD 240·FLY 450·FLY_STEP 22)을 15개에 그대로 쓰면 2.07초로 한도를 넘는다. 현재 44/180/430/12 = 약 1.45초. **칩을 더 늘리면 이 값들을 줄일 것.** ⚠️ reduced-motion·구형에선 이동 없이 결과 화면만 성립한다(요점은 '칩이 움직였다'가 아니라 '열 개가 세 덩어리가 됐다'). ⚠️ `.sort-pool[hidden]{display:none}` 가드 필수 — `display:grid`가 UA `[hidden]`을 이긴다(`.door[hidden]`과 같은 함정).
  - **⚠️⚠️ 자리 미리 잡기(`reserve()`)를 지우지 말 것 — 없으면 스크롤 중 화면이 튄다(2026-07-30 오너 신고 "밑으로 내릴 때 화면이 튀는 것 같다").** 칩 한 덩어리와 세 그룹 카드는 높이가 다르다(칩 10개 시절 실측 375px 262→524 · 1280px 252→524 · 320px 262→524. **15개로 늘린 뒤: 375px 예약 704 · 1280px 예약 712**). 그래서 칩이 그룹으로 바뀌는 **한 프레임에 아래 내용 전부가 262~272px 아래로 순간이동**했다. 원인은 애니메이션 성능이 아니라 **문서 높이가 갑자기 늘어난 것**이다(칩 이동은 `transform` 이라 레이아웃을 안 건드린다 — 여기를 헷갈리면 엉뚱하게 애니메이션을 줄이게 된다). 지금은 두 상태 중 큰 높이를 `wrap.style.minHeight` 로 미리 잡아 정렬 전·후 높이가 같다(**실측 이동량 0px** — 문 섹션 위치·문서 높이가 정렬 전후로 동일).
    ...
    - ⚠️ `min-height` 는 **CSS 가 아니라 JS 로** 준다 — JS 가 죽은 환경에선 칩이 그룹으로 바뀌지 않으므로 CSS 에 박으면 그 자리가 영영 빈 칸이다.
...
- **③ 세 개의 문 = 타임라인**: **매일(승준노트→`briefing.html`) → 2주(챌린지→`challenges.html`) → 면접 직전(특강→`lectures.html`)**. 각 문 위에 `.tl-when` 라벨 + 앞 카드와 잇는 짧은 세로선(`.tl-step + .tl-step::before`)이 붙어 나열이 순서가 된다. **⚠️ 문 순서가 구 스케치(챌린지→승준노트→특강)와 다르다** — 되돌리면 ②의 그룹 순서·타임라인 라벨과 전부 어긋난다. 사진 카드가 좌우로 번갈아 붙는 리듬은 유지(2번째만 `rev`). **⚠️ 카드 자리는 반드시 사진**(텍스트 목록·미니 UI 덩어리는 오너 2회 기각). 사진 3장은 **2026-07-30 오너 제공 최종본으로 교체 완료**(`images/door-briefing.webp` 책상·노트 / `door-challenge.webp` 유니폼 정면 / `door-lecture.webp` 실제 강의실 — 구 `hero-sky-sunset`·`hero-expression`·`instructor-kwon` 재사용 샘플은 폐기). **⚠️ 사진별 `object-position` 가로 초점(`.door-1` 66% · `.door-3` 72%)을 지우지 말 것** — 모바일 카드가 132×220~244 라 cover 가 원본 가로의 **34~45% 만** 남기므로, `center` 로 두면 승준노트 인물이 반으로 잘리고 특강은 강사가 사라져 'TV 화면'으로 읽힌다(실측). 챌린지는 정면 인물이 이미 가운데라 center 유지. 사진을 또 바꾸면 이 두 값을 다시 잡을 것. ⚠️ 특강은 `#doorLecture`(=`.tl-step`)가 `hidden` 시작, 공개 특강이 있을 때만 JS가 켠다 — **C안을 고른 이유 중 하나가 이것**으로, 문이 하나 빠져도 '매일 → 2주'만으로 순서가 성립한다. `.tl-step[hidden]`·`.door[hidden]` 가드 둘 다 유지. 문 칩은 **세 개 모두 실데이터**(승준노트 = 오늘 news_articles count, 챌린지 = 모집중 챌린지 수, 특강 = 가장 임박한 신청중 특강 D-n) — 숫자를 꾸며내지 않는다. 2026-07-30 까지 챌린지 칩만 `DAY 7/14` 라는 지어낸 값이었다(제거 — 아래 항목 참조).
  - **⚠️⚠️ 사진 위 라벨(`.dr-chip`)에 상자를 씌우지 말 것 — 배경·테두리·`border-radius` 전부(2026-07-30 오너 "알약이 아니라 드럼통 같다").** 원인은 문구가 아니라 폭이다: 사진 칸이 375px에서 **실측 132px**이라 한 줄에 한글 7자뿐인데, 12~13자 문구가 두 줄로 풀리면서 `border-radius:999px`(반원)가 **122×53px(2.3:1)짜리 둥근 통**이 됐다. **알약은 한 줄일 때만 알약이다.** 지금은 상자를 없애고 **스크림(`.dr-card::after` 그라디언트) + `— ` 대시 + 흰 글자**다(목업 4안 중 C안 — `outputs/door-chip-mockup.html`이 결정 기록, 기각안 A 짧은 알약·B 아래 띠·D 숫자 태그도 그 파일에 보존). 상자가 없으면 두 줄이 돼도 그냥 두 줄이라 **문구 길이를 관리할 필요 자체가 없어진다.**
    - ⚠️ 스크림은 `pointer-events:none` 필수(`.dr-card`가 `<a>`라 덮으면 클릭이 죽는다) · 칩은 `z-index:2`(둘 다 카드의 자식) · 폭은 `width`가 아니라 `left+right`(둘째 줄이 카드 밖으로 안 나가게).
    ...
    - **⚠️⚠️ 스크림 세기 `.94/.66` 은 대비 손잡이다 — 사진이 어둡다고 낮추지 말 것(2026-07-30 사진 교체 때 재실측).** 구 `.84/.56` 에서 **하늘색 강조(대시 `— `·`<b>숫자</b>`)가 모바일 4.11~4.49 / 데스크톱 3.64~3.92 로 9대 원칙 4번(4.5:1) 미달**이었다. 새 사진 셋 중 둘이 하단이 흰 책상·흰 유니폼이라 더 빠듯해졌지만 **구 사진도 이미 4.32 로 미달**이라 교체가 만든 회귀가 아니다 — 구 주석의 '5.3' 은 흰 글자 수치를 강조색 수치로 착각해 적어 둔 것. 현재 실측(최악 픽셀·그림자 제외): **하늘색 5.14~6.06 / 흰 글자 8.93~10.53**.
...
- **④ 숫자 — 몬크가 쌓아온 것(2026-07-30 오너 확정)**: 라벨 The Numbers → 제목 "어렵고 고민되는 승무원 준비 / 방향을 알려드리겠습니다." → **타일 4개 2×2**(`#moncStats`: 11년 몬크 데이터 · 30,000+명 함께했던 승준생 · 3,500+명 몬크 수강생 · 10,000+개 합격 자소서 데이터) → 마무리 "여러분들의 승무원 준비 여정 / 몬크가 함께 합니다." 숫자·문구 전부 오너 확정 값(출처 대응은 오너 몫).
  - **⚠️⚠️ 구 '업계 현실' 3타일(2만 준비생 · 350여 명 합격 · 0.18% 가능성)과 브릿지("이 숫자를 바꾸는 건 준비한 시간입니다.")는 오너 지시로 삭제** — *"기존 승무원 준비 확률은 빼버려"*. **되살리지 말 것.** 같은 날 아침엔 '현실 3개 → 브릿지 → 몬크 4개' 두 묶음이었는데, 오너가 그 구도(남의 숫자로 겁을 준 뒤 우리 숫자를 대는 것) 자체를 접었다. 지금 이 섹션은 **우리가 쌓아온 것만** 말한다.
  - **⚠️ 그래서 이 섹션엔 액센트 타일이 없다** — `.num.is-hot` CSS도 함께 삭제했다(유일하게 쓰던 0.18% 타일이 사라져 죽은 코드가 됐다). 남은 넷 중 하나를 골라 색을 칠하지 말 것: 넷 다 같은 급의 사실이고, 홈의 액센트 세 자리(라벨·숫자 하나·링크) 중 '숫자 하나'가 비었을 뿐이다.
  - **⚠️⚠️ 2×2 고정 — 데스크톱에서 4열로 펴지 말 것.** 720px 컨테이너를 4열로 나누면 타일 안쪽이 131.5px 인데 여섯 자리 숫자+단위가 38px 활자로 그보다 넓다(구 '30,000+여 명' 실측 159.3px = 숫자 105.7 + '+' 22.9 + 단위 30.7). 들어가게 하려면 26px 까지 내려야 하는데 그러면 숫자가 이 섹션의 주인공 자리를 잃는다. 2열이면 안쪽 309px 라 38px 그대로 넉넉하다(원칙 10). **단위에서 '여'를 뺀 뒤로는 320px 에서도 22px 로 안 깨진다** — 구 ≤360px 축소 블록은 그래서 삭제했다.
  - **⚠️ 카운트업 연출(2026-07-30 오너 "있어보이게, 너무 짧지 않게")**: 2.0초 + easeOutQuint + **타일마다 0.17초 지연**. 구 1.1초 easeOutCubic 동시 시작은 눈이 좇기 전에 끝나 '바뀌었다'로 보였다. 단위(년·명·개·+)는 **숫자가 멈춘 뒤에** 뜬다(`.is-done`) — 굴러가는 동안 '0명' 같은 조합을 감추는 장치이자 착지의 마침표. **⚠️ 단위를 숨기는 CSS 는 `.is-armed`(JS가 붙인다) 안에서만 동작한다** — JS가 죽은 환경에서 단위가 영영 안 보이면 안 된다. 1,000 이상은 콤마(`toLocaleString`), `tabular-nums` 라 자릿수가 늘어도 폭이 안 출렁인다.
...
- ⚠️⚠️ **취업 무관 보도 제외(2026-07-30 오너 지시 "취업이랑 아무런 연관이 없잖아")** — 뉴스판 최상단이 *"'와 7kg 빠졌다' 대한항공 방출 선수, 한국전력 이적을…"*(프로배구 점보스)였다. 항공사 **이름으로 검색하니 같은 이름의 스포츠단·주식 종목·협찬 기사가 함께 딸려 온다.** `dropReason()`이 사유별로 판정하고, **수집(스텝 2)과 기존 저장분 청소(스텝 6)가 같은 `isDropped()`를 쓴다.**
  ...
  - **⚠️⚠️ 이 필터는 못 걸러서가 아니라 멀쩡한 기사를 걷어내서 망한다**(AI킬러 규칙과 같은 실패 모드). 첫 판에서 실제로 밟은 오탐 4종 — 되돌리지 말 것: **`골프`**(→"에어서울, 항공·숙박·골프 묶은 원스톱 여행"이 죽는다. 골프단 협찬만 좁게) · **`제주항공우`의 `(?![가-힣])`**(→"제주항공**우주**박물관"이 우선주로 잘린다) · **`mentionsAirline`**(→`진에어`·`에어프레미아`·`티웨이`처럼 이름에 '항공'이 없는 6개사 기사가 전부 잘린다) · **`한진칼`·`한진그룹`**(→대한항공 지주사 경영권 분쟁은 면접 단골인데 제목에 '항공'이 없다). 단독 금지 낱말: `방출`("탄소 방출") · `선수`("선수금") · `감독`("금융감독원") · `에어`("에어컨").
  - **⚠️ `이스타` 별칭**은 제목의 줄인 표기("이스타-관광공사…") 때문에 필요하다 — 빼면 그 기사가 '항공무관'으로 잘린다. 슬러그는 그대로라 news.html·mypage 라벨 계약에 영향 없다.
  - **검증 실측(2026-07-30, 실데이터 1,126건)**: 제외 15건(스포츠 2·항공무관 7·시황 1·블로그 1·참사 4), 오탐 0. 규칙 손보기 전 첫 판은 219건을 걷어냈고 그중 상당수가 오탐이었다 — **숫자가 크면 잘 걸러진 게 아니라 잘못 걸러진 것이다.**
...
- **⚠️ `.bq-btn` hover 배경을 `--action-hover`(네이비)로 두지 말 것** — 글씨가 `--accent-ink`(네이비)라 마우스를 올리는 순간 네이비 위 네이비로 글자가 사라진다(2026-07-30 수정). 다크 무대 위 버튼은 '흰 알약 + 네이비 글씨'가 규칙이니 hover 도 흰 계열 안에서 한 단계만 누른다(`#E8EEF8`).
...
- **안무(2026-07-30 오너 "조립을 더 부드럽고 빠르게" 로 재조정)**: 0~0.4초 문장 정지(읽는 시간) → 0.4~1.35초 **조립** → 1.3초 한글 → 1.8초 스크롤 신호. 총 **2.45초**, **로드 직후 rAF 1회**. (구 값 0.6 / 1.8초 조립 / 총 3.6초)
  - **⚠️⚠️ 시간만 줄이지 말고 이징을 함께 볼 것 — '느리다'의 진짜 원인은 곡선이었다.** 구 `easeInOutCubic`(대칭)은 정지 상태에서 **속도 0으로 출발**해 앞의 20%를 기어가고(1.8초 중 360ms 동안 고작 3% 이동) 끝에서도 같은 만큼 기어서, 실제 이동이 가운데 짧은 구간에 몰려 있었다. 대칭 곡선을 그대로 두고 시간만 줄이면 그 가운데만 빨라져 **'휙 지나갔는데 여전히 늑장'** 이 된다. 지금은 **`easeOutCubic`** — 바로 출발해 길게 미끄러져 착지한다(첫 100ms 에 28%, 절반 시점에 88%, 남은 12%를 길게 흘림). 도착 속도가 0 이라 '툭' 끝나지도 않는다. **대칭 이징으로 되돌리지 말 것.**
