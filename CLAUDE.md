...
- **④ 숫자 — 두 묶음(2026-07-30 오너 요청으로 확장)**: 제목 "어렵고 고민되는 승무원 준비 / 방향을 알려드리겠습니다." → **㉮ 업계 현실 3개**(`#numStats`: 2만 준비생 · 350여 명 합격 · 0.18% 가능성) → 브릿지 "이 숫자를 바꾸는 건 준비한 시간입니다." → **㉯ 몬크가 쌓아온 것 4개**(`#moncStats`: 11년 몬크 데이터 · 30,000+여 명 함께한 승준생 · 3,500+여 명 수강생 · 10,000+여 개 합격 자소서 데이터) → 마무리 "여러분들의 승무원 준비 여정 / 몬크가 함께 합니다." 숫자·문구 전부 **오너 확정 값**(출처 대응은 오너 몫). **⚠️ 숫자는 삭제된 게 아니라 ①에서 여기로 이사한 것** — 감정 뒤에 사실이 와야 더 세게 읽힌다는 것이 C안의 전제다.
  - **⚠️ 순서를 뒤집지 말 것(현실 → 우리)** — 몬크 숫자를 먼저 놓으면 자랑이 되고, 현실을 먼저 보여야 "그래서 우리가 이만큼 준비했다"로 읽힌다. 두 묶음 사이의 브릿지 문장이 그 이음매라 지우면 일곱 타일이 한 덩어리로 뭉갠다.
  - **⚠️ 액센트(`is-hot`)는 0.18% 한 곳뿐** — 몬크 타일 4개에 색을 붙이지 말 것(팔레트 규칙: 홈의 액센트는 라벨·숫자 하나·링크 세 자리). 강조가 다섯이면 강조가 없는 것과 같다.
  - **⚠️⚠️ 몬크 4타일은 어느 폭에서도 2×2다 — 데스크톱에서 4열로 펴지 말 것.** 720px 컨테이너 4열이면 타일 안쪽이 131.5px 인데 '30,000+여 명'은 38px 활자로 **159.3px**(숫자 105.7 + '+' 22.9 + '여 명' 30.7 — 실측)이라 단위가 다음 줄로 떨어진다. 32px 로 낮춰도 넘치고, 들어가게 하려면 26px 인데 그러면 위 3타일(38px)과 숫자 크기가 따로 논다. 자릿수가 줄기 전엔 4열이 성립하지 않는다. ≤360px 은 활자 20px + 좌우 여백 5px 로 한 단계 더 내린다(360px 에서 3px 이 모자라 같은 증상이 난다).
  - **⚠️ 카운트업 연출(2026-07-30 오너 "있어보이게, 너무 짧지 않게")**: 2.0초 + easeOutQuint + **타일마다 0.17초 지연**. 구 1.1초 easeOutCubic 동시 시작은 눈이 좇기 전에 끝나 '바뀌었다'로 보였다. 단위(년·여 명·%·+)는 **숫자가 멈춘 뒤에** 뜬다(`.is-done`) — 굴러가는 동안 '0여 명' 같은 조합을 감추는 장치이자 착지의 마침표. **⚠️ 단위를 숨기는 CSS 는 `.is-armed`(JS가 붙인다) 안에서만 동작한다** — JS가 죽은 환경에서 단위가 영영 안 보이면 안 된다. 1,000 이상은 콤마(`toLocaleString`), `tabular-nums` 라 자릿수가 늘어도 폭이 안 출렁인다.
