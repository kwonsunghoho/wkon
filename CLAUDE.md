...
- Active detail pages (challenges.html 카드에서 링크, `application-modal.js` 로드하나 신청은 `apply.html?c=<id>`로): `challenge-voice.html`(보신각), `challenge-expression.html`(영합각), `challenge-spinning.html`(스피닝), `challenge-answer.html`(승자각). ⚠️ 블라인드 퀴즈는 2026-07-30 허브(challenges.html) 하단으로 이사 — 상세엔 없다.
...
- **② 열 가지 → 세 가지**("혼자하면 막막하지만, 몬크와 함께하면 **간결합니다**." — 2026-07-30 오너 문구): 칩 10개(자기소개서·보이스·스피치·이미지연출·기업분석·카메라연출·필수기출·롤플레잉·영어인터뷰·멘탈관리 — **오너 확정 목록**)가 0.28초 간격으로 하나씩 체크된 뒤, **같은 칩이 제자리에서 세 그룹으로 날아가 붙는다**(FLIP). `data-g` 1=승준노트 2=챌린지 3=특강 → **③의 문 순서와 같다**. ⚠️ 칩 나열 순서는 일부러 섞여 있다(그룹 순으로 늘어놓으면 '정리되는 순간'이 안 보인다). ⚠️ 그룹 제목은 문 이름과 **글자 그대로 같아야** 방금 정리한 세 덩어리가 아래 세 문과 같은 것으로 읽힌다. ⚠️ **한 번만 실행**(`io.disconnect`) — 되감기를 넣으면 스크롤을 오르내릴 때마다 글이 다시 움직인다. ⚠️ reduced-motion·구형에선 이동 없이 결과 화면만 성립한다(요점은 '칩이 움직였다'가 아니라 '열 개가 세 덩어리가 됐다'). ⚠️ `.sort-pool[hidden]{display:none}` 가드 필수 — `display:grid`가 UA `[hidden]`을 이긴다(`.door[hidden]`과 같은 함정).
  - **⚠️⚠️ 자리 미리 잡기(`reserve()`)를 지우지 말 것 — 없으면 스크롤 중 화면이 튄다(2026-07-30 오너 신고 "밑으로 내릴 때 화면이 튀는 것 같다").** 칩 한 덩어리와 세 그룹 카드는 높이가 다르다(**실측 375px 262→524 · 1280px 252→524 · 320px 262→524**). 그래서 칩이 그룹으로 바뀌는 **한 프레임에 아래 내용 전부가 262~272px 아래로 순간이동**했다. 원인은 애니메이션 성능이 아니라 **문서 높이가 갑자기 늘어난 것**이다(칩 이동은 `transform` 이라 레이아웃을 안 건드린다 — 여기를 헷갈리면 엉뚱하게 애니메이션을 줄이게 된다). 지금은 두 상태 중 큰 높이를 `wrap.style.minHeight` 로 미리 잡아 정렬 전·후 높이가 같다(**실측 이동량 0px** — 문 섹션 위치·문서 높이가 정렬 전후로 동일).
    ...
    - 대가로 정렬 전 1.5초 동안 칩 아래 **약 262~273px 의 빈 자리**가 보인다. 이건 버그가 아니라 '움직임 0'과 맞바꾼 것이다 — 없애려면 다시 튀는 수밖에 없다(대안은 높이를 부드럽게 늘리는 것인데, 그래도 아래 내용은 움직인다).
- **③ 세 개의 문 = 타임라인**: **매일(승준노트→`briefing.html`) → 2주(챌린지→`challenges.html`) → 면접 직전(특강→`lectures.html`)**. 각 문 위에 `.tl-when` 라벨 + 앞 카드와 잇는 짧은 세로선(`.tl-step + .tl-step::before`)이 붙어 나열이 순서가 된다. **⚠️ 문 순서가 구 스케치(챌린지→승준노트→특강)와 다르다** — 되돌리면 ②의 그룹 순서·타임라인 라벨과 전부 어긋난다. 사진 카드가 좌우로 번갈아 붙는 리듬은 유지(2번째만 `rev`). **⚠️ 카드 자리는 반드시 사진**(텍스트 목록·미니 UI 덩어리는 오너 2회 기각). 사진 3장은 **샘플 — 최종본 오면 교체**. ⚠️ 특강은 `#doorLecture`(=`.tl-step`)가 `hidden` 시작, 공개 특강이 있을 때만 JS가 켠다 — **C안을 고른 이유 중 하나가 이것**으로, 문이 하나 빠져도 '매일 → 2주'만으로 순서가 성립한다. `.tl-step[hidden]`·`.door[hidden]` 가드 둘 다 유지. 문 칩은 **실데이터만**(승준노트 = 오늘 news_articles count, 특강 = 가장 임박한 신청중 특강 D-n) — 숫자를 꾸며내지 않는다.
  - **⚠️⚠️ 사진 위 라벨(`.dr-chip`)에 상자를 씌우지 말 것 — 배경·테두리·`border-radius` 전부(2026-07-30 오너 "알약이 아니라 드럼통 같다").** 원인은 문구가 아니라 폭이다: 사진 칸이 375px에서 **실측 132px**이라 한 줄에 한글 7자뿐인데, 12~13자 문구가 두 줄로 풀리면서 `border-radius:999px`(반원)가 **122×53px(2.3:1)짜리 둥근 통**이 됐다. **알약은 한 줄일 때만 알약이다.** 지금은 상자를 없애고 **스크림(`.dr-card::after` 그라디언트) + `— ` 대시 + 흰 글자**다(목업 4안 중 C안 — `outputs/door-chip-mockup.html`이 결정 기록, 기각안 A 짧은 알약·B 아래 띠·D 숫자 태그도 그 파일에 보존). 상자가 없으면 두 줄이 돼도 그냥 두 줄이라 **문구 길이를 관리할 필요 자체가 없어진다.**
    ...
    - ⚠️ `text-shadow`를 지우지 말 것 — 윗줄은 스크림이 옅어지는 구간에 걸친다. **실측 대비(그림자 없이 계산한 보수값): 375px에서 11.8 / 5.3 / 8.5 : 1, 데스크톱 9.6 / 6.2 / 7.5 : 1** — 가장 빠듯한 게 영합각 사진(5.3)이라 스크림을 더 옅게 만들면 9대 원칙 4번(4.5:1)이 깨진다.
    - ⚠️ **D안(숫자 태그)을 나중에 다시 꺼낸다면 챌린지 칸의 진짜 숫자부터 정할 것** — 'DAY 7/14'는 지어낸 값이라 크게 키우면 안 된다(뉴스 건수·특강 D-day만 실데이터다).
...
- **타이포:** 섹션 제목은 `h2.section-title/.ts-title/.mc-title/.ma-title`에 `var(--serif)`(=SUIT, 이름만 명조 시절 흔적) 800 + `--fs-h2 clamp(30px,4.2vw,46px)` (tokens.css 오버라이드, h2 접두 특이도로 `index.css`의 `.section-title` 규칙을 이김).
...
자기 주입 공용 컴포넌트(application-modal.js 패턴): **`challenges.html` 한 곳만** `<div id="blind-quiz-mount">`(카드 4장 뒤·'홈으로 돌아가기' 앞) + `<script src="blind-quiz.js" defer>`로 싣는다.
