...
- **⚠️ 데이터 소스는 Supabase `challenge_rounds` 우선, 없는 챌린지만 CSV 보충(2026-07-23):** `loadRecruitData()`가 Supabase 결과에서 `RECRUIT_FALLBACKS` 키 중 빠진 챌린지를 감지해 구글 시트로 메운다. 이게 없으면 **Supabase에 일부 기수만 등록된 상태**(예: expression·answer만 4기 등록, voice·spinning 미등록)에서 미등록 챌린지가 페이지마다 제각각인 HTML 하드코딩 날짜로 떨어져 **메인·상세 모집기간이 어긋난다**(실제로 겪은 자리 — 상세 voice 5/1 vs 메인 6/1). 보충 로직·상세 폴백 동기화 되돌리지 말 것. **미등록 챌린지의 올바른 날짜는 admin '모집일정' 탭에서 기수를 등록하면 자동 반영**(코드가 아니라 데이터 문제).
