...
- **⚠️ 데스크톱(≥1024px) — 스토리 컬럼 확대(2026-07-29 오너 "웹인데 따닥따닥")**: 구조는 그대로 두고 폭·활자만 큰 화면 급으로 올린다(미션 560→720px, 문 640→980px, 문 제목 22→30px·설명 14→17px, 사진은 폭과 같은 배율 ×1.53). 개편 직후엔 브레이크포인트가 720px 하나뿐이라 **1440px에서도 375px 모바일 값(문 제목 22px·설명 14px)이 그대로 떴다** — `clamp()`가 미션·마무리 제목에만 걸려 있어 문 섹션은 아예 커지지 않았다. ⚠️ **문 3개를 3열 카드로 펼치지 말 것(오너 기각)** — 좌우 교차·높이 차의 어긋난 리듬이 이 섹션의 구도이고, 3열이면 challenges.html 카드 목록과 구분이 사라진다. ⚠️ `.door`의 `max-width`는 `min(980px, calc(100% - 96px))` — 980px 고정이면 브레이크포인트가 시작되는 1024px에서 좌우가 19px만 남는다(실측). ⚠️ 사진 높이만 키우지 말 것(비율이 무너져 인물이 잘린다).
