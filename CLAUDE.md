...
### Pages
- `index.html` — the landing page. **2026-07-29 스토리형 개편으로 대폭 축소**(인트로 → 왜 몬크인가 → 세 개의 문 → 마무리 CTA — 아래 '랜딩 섹션 순서' 참조). **CSS는 `index.css`로 분리** — 캐스케이드 `tokens.css`→`index.css` 순서 보존이 핵심(링크 순서 바꾸면 `!important` 싸움이 깨짐). ⚠️ index의 CSS는 `index.css`에서 찾을 것(index.html 안엔 `<style>` 없음). JS는 여전히 index.html 인라인.
...
### 랜딩 섹션 순서 — 2026-07-29 스토리형 개편(오너 스케치 확정 · `outputs/homepage-story-mockup.html` v4가 결정 기록)
창문 인트로(유지) → **왜 몬크인가(`.msn`, `id="home"`)** → **세 개의 문(`.doors`)** → **마무리 CTA(`.closing`)**. 개편 목적(오너): "홈페이지에 스토리가 없이 기능들만 나열해놓은 느낌" — 현실(숫자) → 가능성 → 몬크의 세 가지 답 순서로 재구성.
- **왜 몬크인가**: 미션 한 문장("어렵고 고민되는 승무원 준비를 돕기 위해 시작했습니다") + 숫자 3타일(**20만 준비생 · 매년 500 합격 · 0.25% 가능성** — 오너 스케치 값, 출처 확정은 오너 몫) + 브릿지("그 가능성을, 몬크에서 도전하세요"). 다크 무대 + 카운트업 1회. **⚠️ `id="home"`이 이 섹션에 있다** — 하단 CTA 바 '몬크 더 알아보기'(.is-browse) 착지점·footer '서비스 소개'·`hero_reached` 계측이 전부 이 앵커를 본다.
- **세 개의 문**: 챌린지→`challenges.html` / 브리핑룸→`briefing.html` / 특강→`lectures.html`. 글 옆에 **사진 카드**가 좌우로 번갈아 붙고(door-2만 rev) 모바일에선 화면 끝까지 나가 잘린다. 높이 252/228/206px + 글 시작점도 어긋남(레퍼런스의 '언밸런스'). **⚠️ 카드 자리는 반드시 사진** — 텍스트 목록·미니 UI 덩어리로 채웠다가 오너에게 2회 기각된 자리("이미지가 들어갈 자리에 텍스트로 떡칠"). 현재 사진 3장(영합각·하늘·권성호)은 **샘플 — 오너가 최종 이미지를 주면 교체**. ⚠️ 특강 문은 `hidden` 시작, 공개 특강이 있을 때만 JS가 켠다(빈 방 금지 — 구 #pillars 규칙 승계). `.door[hidden]{display:none}` 가드 필수(display:flex가 UA [hidden]을 이긴다 — 실제로 밟은 함정). 문 칩은 **실데이터만**(브리핑룸 = 오늘 news_articles count "오늘 뉴스 N건", 특강 = 가장 임박한 신청중 특강 D-n) — 숫자를 꾸며내지 않는다(조회 실패 시 기본 문구 유지).
- **마무리 CTA**: 미션을 받는 수미상관("가능성은, 매일 준비한 사람에게 옵니다") + '몬크 시작하기'→apply.html + '이미 회원이라면 로그인' 한 줄. 구 성장기록 섹션의 회원가입 유도 역할을 흡수.
- **⚠️⚠️ 홈 팔레트 = 딥 네이비 on 웜 페이퍼(2026-07-29 오너 확정 · 목업 `outputs/home-color-mockup.html` B안이 결정 기록, 기각안 A·C도 그 파일에 보존)**: 오너 지시 *"폰트에 색을 너무 많이 추가한 것 같다 / 오렌지는 탈피 / 첫 이미지처럼 깔끔·단정하게"*. **`index.css` 최상단 `:root` 오버라이드**가 단일 소스 — `--action`·`--accent`·`--accent-dark`·`--accent-ink`를 **전부 `#1B3A6B` 한 값으로 모았다**(세 단계로 나뉘어 있으면 자리마다 다른 채도가 섞여 다시 산만해진다). 배경도 `--bg #FBF9F5` / `--bg2 #F4F1EA`로 밝고 채도를 낮췄고, 다크 무대 `--bg-dark`는 갈색에서 잉크 네이비 `#1C2A3A`로.
  ...
  - **⚠️ 브리핑룸 문(`.door.rev`)은 글도 오른쪽 정렬**(2026-07-29 오너 지적 "브리핑룸이 변칙인데 왜 텍스트는 왼쪽 정렬이냐"). 사진이 왼쪽·글이 오른쪽인 카드라, 왼쪽 정렬로 두면 글 블록 오른쪽에 빈 띠가 남아 카드가 한쪽으로 쏠려 보인다. 라벨 대시(`::before`)와 링크 화살표는 inline 요소라 자동으로 따라간다.
...
### 히어로 — 창문 정적 배경 + 로고 흩날림 (`#heroIntro` · 2026-07-29 전면 교체)
**구 '창문 인트로 + MONC 조립 + 창 통과 줌'은 통째로 폐기됐다.** 오너가 새 배경 이미지(창+하늘 **합본**)를 주면서 연출을 뒤집었다: *"스크롤 당기면 글자가 모이는 걸 반대로 / 애니메이션 없이 정적 이미지로"*. 결정 기록은 `outputs/hero-sand-mockup.html`(오너가 슬라이더로 값을 직접 고른 목업).

- **구성**: `<section class="hero-intro">` = `<picture>` 배경 한 장 + 정중앙 `#heroMid`(MONC 로고 + 한글 한 줄) + `#heroCue`(SCROLL). 높이 **100svh 한 장**.
- **⚠️ 스크롤과 무관하다.** 러웨이(200/180vh)·sticky 핀·재방문 단축(`data-zoom-runway` 재작성)·`monc:zoomprogress` 이벤트가 전부 사라졌다. 애니메이션은 **로드 직후 rAF 1회 재생**. 인트로 도달률 38%의 원인이던 긴 스크롤 구간 자체가 없어진다(index.html 1353→717줄).
- **안무(오너 확정)**: 1.2초 정지(읽는 시간) → 0.9초 **밝아짐**(네이비 `#1B3A6B` → 선명한 블루 `#2E63B8` + 빛 번짐 확대) → 1.7초 **위로 스르륵 사라짐** → 3.45초 SCROLL 등장. 총 4.2초.
- **⚠️⚠️ 방향은 하나(위)다 — 좌우로 벌리지 말 것.** 초안은 글자를 바깥으로 흩었는데 오너가 *"왜 제각 흐려지냐, 한 방향으로 스르륵 사라져야지"*로 기각했다. 좌우 이동은 흔들림 수준(±20px), 회전 ±3.5°. 실측상 로고 가로폭이 129→128px로 **그대로**여야 맞다(벌어지면 잘못된 것).
- **⚠️ 수치는 오너가 목업 슬라이더로 고른 확정값**: 올라가는 거리 **50** · 글자 시차 **0** · 흐려짐 **8.0**. 감으로 바꾸지 말 것.
- **⚠️ 정중앙 = 창 한가운데.** 창 개구부를 이미지에서 실측하니 세로 중심이 **모바일 50.0% / 웹 52.6%**라 두 값이 같은 자리다(오너 *"정중앙에서 뭔가 보였으면"*). 웹만 `.hero-mid { top: 51.5% }`로 보정.
- **⚠️ 글자색은 네이비 유지 · 밝아짐을 흰색으로 가져가지 말 것.** 창 정중앙의 하늘이 **`#E4EFF9`로 거의 흰색**이라(창 위쪽은 `#A4C5E5`) 흰 글자는 그 자리에서 사라진다. 밝아짐은 **채도 상승 + 글로우 확대**로 표현한다.
- **⚠️ 로고를 두 번 보여주지 말 것.** 초안은 흩어진 뒤 나오는 영문 문장에서 M·O·N·C를 굵게 강조했는데 오너가 *"로고가 두 번 나온다"*로 지적 — 영문 문장 자체를 뺐다(지금은 로고 + 한글 한 줄뿐).
- **⚠️ `prefers-reduced-motion`이면 글자를 지우지 않는다.** 애니메이션만 건너뛰고 정지 화면으로 둔다 — 움직임을 못 보는 사람에게서 브랜드 메시지까지 뺏지 않기 위해. **이 분기를 '아무것도 안 함'으로 바꾸면 그 사람들은 빈 창 사진만 보게 된다.**
- **⚠️ 배경 이미지가 뜬 뒤 시작한다**(`load`/`error` + 1.2초 안전망). 안 그러면 느린 회선에서 글자만 떠 있는 화면이 보인다.
- **이미지**: `images/hero-window-web.webp`(1717×916 · 60KB) / `images/hero-window-mob.webp`(853×1844 · 67KB). **⚠️ 분기는 폭이 아니라 `(max-aspect-ratio: 1/1)`** — 태블릿 세로에서 가로형 이미지를 cover 하면 좌우가 심하게 잘려 창만 거대해진다. 모바일 이미지 비율 1:2.16이 375×812(1:2.165)와 거의 일치해 잘림이 없다.
- **⚠️ 새 배경으로는 '창 통과 줌'을 되살릴 수 없다** — 창과 하늘이 한 장에 합쳐져 있어 확대하면 그림 전체가 커질 뿐이다. 되살리려면 하늘/창틀 2겹 이미지부터 필요하다.
- **함께 삭제된 것**: `scroll-fx.js`의 `initZoomExit`(293줄) · `tokens.css`의 `[data-zoom-exit]`/`.zoom-exit-pin` 규칙 · 태그라인 FLIP IIFE(약 35KB) · 창틀·하늘 이미지 2겹 · 페이드 레이어 4장. 구 스펙 문서(`docs/superpowers/specs/2026-07-10-hero-monc-tagline-design.md` 등)는 이제 **역사 기록**이다.
