...
- `ai-killer.html` — **AI킬러(2026-07-25 신설).** 자소서·답변에서 'AI 같은 표현'을 찾아 밑줄 + [왜/고치기] 칸으로 돌려준다. 승준노트 카드에서 진입, **로그인 + 동의 게이트 필수**. 전체 스펙: `docs/superpowers/specs/2026-07-24-ai-killer-design.md`(소스오브트루스). 요약만:
  ...
  - **규칙을 고치면 `node scripts/ai-killer-dryrun.mjs`로 기준선과 비교할 것**(DB·API 없이 규칙만). 특히 '사람이 잘 쓴 글'이 0곳을 유지해야 한다 — 규칙 검사기는 못 잡아서가 아니라 정상적인 글에 밑줄을 그어서 신뢰를 잃는다.
...
- **③ 세 개의 문 = 타임라인**: **매일(승준노트→`briefing.html`) → 2주(챌린지→`challenges.html`) → 면접 직전(특강→`lectures.html`)**. 각 문 위에 `.tl-when` 라벨 + 앞 카드와 잇는 짧은 세로선(`.tl-step + .tl-step::before`)이 붙어 나열이 순서가 된다. **⚠️ 문 순서가 구 스케치(챌린지→승준노트→특강)와 다르다** — 되돌리면 ②의 그룹 순서·타임라인 라벨과 전부 어긋난다. 사진 카드가 좌우로 번갈아 붙는 리듬은 유지(2번째만 `rev`). **⚠️ 카드 자리는 반드시 사진**(텍스트 목록·미니 UI 덩어리는 오너 2회 기각). 사진 3장은 **2026-07-30 오너 제공 최종본으로 교체 완료**(`images/door-briefing.webp` 책상·노트 / `door-challenge.webp` 유니폼 정면 / `door-lecture.webp` 실제 강의실 — 구 `hero-sky-sunset`·`hero-expression`·`instructor-kwon` 재사용 샘플은 폐기). **⚠️ 사진별 `object-position` 가로 초점(`.door-1` 66% · `.door-3` 72%)을 지우지 말 것** — 모바일 카드가 132×220~244 라 cover 가 원본 가로의 **34~45% 만** 남기므로, `center` 로 두면 승준노트 인물이 반으로 잘리고 특강은 강사가 사라져 'TV 화면'으로 읽힌다(실측). 챌린지는 정면 인물이 이미 가운데라 center 유지. 사진을 또 바꾸면 이 두 값을 다시 잡을 것. ⚠️ 특강은 `#doorLecture`(=`.tl-step`)가 `hidden` 시작, 공개 특강이 있을 때만 JS가 켠다 — **C안을 고른 이유 중 하나가 이것**으로, 문이 하나 빠져도 '매일 → 2주'만으로 순서가 성립한다. `.tl-step[hidden]`·`.door[hidden]` 가드 둘 다 유지. 문 칩은 **세 개 모두 실데이터**(승준노트 = 오늘 news_articles count, 챌린지 = 모집중 챌린지 수, 특강 = 가장 임박한 신청중 특강 D-n) — 숫자를 꾸며내지 않는다. 2026-07-30 까지 챌린지 칩만 `DAY 7/14` 라는 지어낸 값이었다(제거 — 아래 항목 참조).
  - **⚠️⚠️ 사진 위 라벨(`.dr-chip`)에 상자를 씌우지 말 것 — 배경·테두리·`border-radius` 전부(2026-07-30 오너 "알약이 아니라 드럼통 같다").** 원인은 문구가 아니라 폭이다: 사진 칸이 375px에서 **실측 132px**이라 한 줄에 한글 7자뿐인데, 12~13자 문구가 두 줄로 풀리면서 `border-radius:999px`(반원)가 **122×53px(2.3:1)짜리 둥근 통**이 됐다. **알약은 한 줄일 때만 알약이다.** 지금은 상자를 없애고 **스크림(`.dr-card::after` 그라디언트) + `— ` 대시 + 흰 글자**다(목업 4안 중 C안 — `outputs/door-chip-mockup.html`이 결정 기록, 기각안 A 짧은 알약·B 아래 띠·D 숫자 태그도 그 파일에 보존). 상자가 없으면 두 줄이 돼도 그냥 두 줄이라 **문구 길이를 관리할 필요 자체가 없어진다.**
    ...
    - ⚠️ **펄스를 여기에 달지 말 것** — nav '승준노트' 옆 펄스 점이 "여기가 주력"이라는 **유일한** 신호라서 값이 있다. 홈 카드에 하나 더 달면 둘 다 배경이 된다. 움직임이 필요하면 깜빡임이 아니라 숫자 카운트업(한 번 보고 끝난다).
...
- **⚠️⚠️ 홈 팔레트 = 딥 네이비 on 웜 페이퍼(2026-07-29 오너 확정 · 목업 `outputs/home-color-mockup.html` B안이 결정 기록, 기각안 A·C도 그 파일에 보존)**: 오너 지시 *"폰트에 색을 너무 많이 추가한 것 같다 / 오렌지는 탈피 / 첫 이미지처럼 깔끔·단정하게"*. **`index.css` 최상단 `:root` 오버라이드**가 단일 소스 — `--action`·`--accent`·`--accent-dark`·`--accent-ink`를 **전부 `#1B3A6B` 한 값으로 모았다**(세 단계로 나뉘어 있으면 자리마다 다른 채도가 섞여 다시 산만해진다). 배경도 `--bg #FBF9F5` / `--bg2 #F4F1EA`로 밝고 채도를 낮췄고, 다크 배경 `--bg-dark`는 갈색에서 잉크 네이비 `#1C2A3A`로.
  ...
  - **2026-07-29 같은 날 전 사이트로 확장 완료**(오너 지시) — 팔레트 본체는 `tokens.css`에 있고, `index.css`의 `:root`엔 **홈에만 필요한 값**(`--ink`·다크 배경·테두리)만 남는다. 인트로 태그라인도 `--ht-mark`로 전환. 전 사이트 규칙은 위 'Design system' 항목 참조.
...
- **nav(2026-07-29)**: '커뮤니티' 메뉴 삭제(섹션 소멸 — researchers.html의 `index.html#community` 링크도 함께 제거). '챌린지' 드롭다운·모바일 아코디언 첫 항목에 '챌린지 한눈에'(→challenges.html) 추가.
...
- **안무(2026-07-30 오너 "조립을 더 부드럽고 빠르게" 로 재조정)**: 0~0.4초 문장 정지(읽는 시간) → 0.4~1.35초 **조립** → 1.3초 한글 → 1.8초 스크롤 신호. 총 **2.45초**, **로드 직후 rAF 1회**. (구 값 0.6 / 1.8초 조립 / 총 3.6초)
...
- **⚠️ 타임라인 `total`은 마지막 페이드(`cueAt + 0.5` = 2.30)보다 커야 한다** — 모자라면 스크롤 신호가 중간 투명도에서 얼어붙는다(구 3.4로 뒀을 때 0.83에서 멈춘 실측). 타임라인을 또 줄이면 이 관계를 먼저 확인할 것.
...
- **레터프레스 종이 카드**: 종이 그라디언트(#FCF9F1→#F3EEE1) + 갈빛 그림자 3겹(inset 1px / 2px / 18px — 하나라도 빼면 종이 질감이 사라진다). ⚠️ 흰 카드 금지(베이지 배경 위 이물감) · 사진 위 글자 금지(크림 막이 사진을 죽인다 — 글자는 사진 밖).
