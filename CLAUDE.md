...
- **전화번호로 남을 조회할 창구를 만들지 않는다** — 비회원 중복 신청 사전 조회, 오픈 알림 명단 본인 조회를 일부러 안 연 이유(번호만으로 신청·관심 여부가 캐진다). **승인된 예외 하나(2026-08-20 오너 확정)**: 회원 프로필 저장의 중복 가입 대조(`save_my_profile` RPC — 로그인 필수·24시간 10회 제한·기존 계정의 로그인 수단까지만 안내). 이 예외를 넓히려면 오너 확인 먼저.
