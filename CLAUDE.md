...
- **휴대폰 본인인증(2026-08-26, KG이니시스 통합인증·포트원 경유)**: 가입 온보딩 + **특강 신청·승준 도구·연구실 자료는 인증 회원만**(`MONC.requireVerified()` 게이트 — 도구 6·특강 3·연구실 2곳, 챌린지 신청·게임·뉴스는 대상 아님). 판정은 서버(verify-identity → `apply_identity_verification` service_role 전용), 인증 회원의 이름·번호는 화면 저장으로 못 바꾼다(save_my_profile 잠금). **게이트는 fail-open** — 안전장치(3중 + **인앱 브라우저 통과** 2026-08-27 오너 확정: 인앱은 인증창의 앱 전환이 막혀 게이트·온보딩 모두 인증을 강요하지 않는다 — 유예일 뿐, 일반 브라우저로 오면 다시 요구)를 빼면 인프라 장애·인앱 유입이 전 기능 잠금이 된다. 채널 키는 `pay-methods.js` `identityChannel` 한 곳. 상세는 auth-consent.md '휴대폰 본인인증' 절. **승인된 예외 하나(2026-08-20 오너 확정)**: 회원 프로필 저장의 중복 가입 대조(`save_my_profile` RPC — 로그인 필수·24시간 10회 제한·기존 계정의 로그인 수단까지만 안내). 이 예외를 넓히려면 오너 확인 먼저.
- **전화번호는 가입 필수 + 구글·카카오 이중 가입 차단(2026-08-20 오너 확정)**: 온보딩 건너뛰기 없음, `requireConsent()` 가입 완성 가드가 번호 없는 회원을 온보딩으로 보낸다. 프로필 저장은 `MONC.saveMyProfile()`(서버 대조) 한 곳 — 직접 `members.update` 로 되돌리지 말 것. 상세는 `docs/notes/auth-consent.md` '전화번호 필수' 절.
...
| 승준 코스·승준 도구 | `briefing.html`(코스)·`tools.html`(도구) | 2026-08-06 분리(구 승준노트 — 파일·클래스명 유지, 화면 글자만 교체). **코스는 2026-08-25 잠시 내림**(사용량 저조 — briefing 은 tools 로 리다이렉트, nav·홈 문·클로징 버튼·to-course·mypage 줄 다섯 곳 한 세트, `members.course` 데이터는 유지 — 복원 절차는 briefing.md '승준 코스 잠시 내림'). 도구는 종이 카드 리스트 허브(2026-08-27 3안 — 구 틴트 박스 격자 대체. 2026-08-25 AI킬러·2026-08-26 일문일답 잠시 내림: 각각 카드·메뉴 한 세트, 표시 5종). 카드·타일에 회원별 상태 문구·금액 금지. nav 도구 드롭다운 — 햄버거 상한 880px 은 코스 내림 후에도 유지 | `docs/notes/briefing.md` |
