...
| 연구실 | `lab.html`(허브)·`lab-archive.html`(원장)·`researchers-data.js` | 허브는 연구실(다크 콘솔)/연구진(종이+얼굴) 카드 두 장뿐(오너 확정), 원장은 카드형(헤어라인 폐지), 숫자는 DB 실측값만(지금은 예시+foot-note) | `docs/notes/lab.md` |
