...
| 승준 코스·승준 도구 | `briefing.html`(코스)·`tools.html`(도구) | 코스는 2026-08-25 잠시 내림(briefing→tools 리다이렉트 — 복원 절차는 briefing.md), 도구 허브는 **맨 위 노트 포스터(2026-09-23 오너 A안 — 홈 챌린지 포스터 문법) + 종이 카드 4종**(2026-08-27 3안 — AI킬러·일문일답 잠시 내림, 순서 소재→첨삭→뉴스→게임 = `nav.js` TOOLS_SUB). 포스터·카드에 회원별 상태 문구·금액 금지 | `docs/notes/briefing.md` |
...
| 연구실 | `lab.html`·`lab-archive.html`·`lab-shelf.html`(서가 4종 공용)·`lab-viewer.js`·`researchers-data.js` | 허브는 '출발 안내판' 한 벌(2026-09-23 · 숫자·날짜·NEW 전부 실값, 마크업에 항공사·숫자 금지 · lab.md '허브 = 출발 안내판'). 상세는 `?shelf=` 한 파일(서가별 HTML 금지), 자료 파일은 비공개 버킷 + lab-file 서명 URL 만, PDF 는 lab-viewer 가 먼저(화면 전용 차단은 서버 유지), 값은 `lab_resources.price`(0=무료) — 채용 캘린더만 `recruit_rounds` 예외. ⚠️ **2026-09-22 서가 접기 — 학생 화면 서가는 취업 자료실 하나**(영상관·현장 리포트·채용 캘린더와 `lab-archive.html` 은 잠시 닫음, 데이터·admin 은 그대로 · lab.md '서가 접기') | `docs/notes/lab.md` |
