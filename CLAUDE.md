...
- **② 열 가지 → 세 가지**("혼자하면 막막하지만, 몬크와 함께하면 **간결합니다**." — 2026-07-30 오너 문구): 칩 10개(자기소개서·보이스·스피치·이미지연출·기업분석·카메라연출·필수기출·롤플레잉·영어인터뷰·멘탈관리 — **오너 확정 목록**)가 0.28초 간격으로 하나씩 체크된 뒤, **같은 칩이 제자리에서 세 그룹으로 날아가 붙는다**(FLIP). `data-g` 1=브리핑룸 2=챌린지 3=특강 → **③의 문 순서와 같다**. ⚠️ 칩 나열 순서는 일부러 섞여 있다(그룹 순으로 늘어놓으면 '정리되는 순간'이 안 보인다). ⚠️ 그룹 제목은 문 이름과 **글자 그대로 같아야** 방금 정리한 세 덩어리가 아래 세 문과 같은 것으로 읽힌다. ⚠️ **한 번만 실행**(`io.disconnect`) — 되감기를 넣으면 스크롤을 오르내릴 때마다 글이 다시 움직인다. ⚠️ reduced-motion·구형에선 이동 없이 결과 화면만 성립한다(요점은 '칩이 움직였다'가 아니라 '열 개가 세 덩어리가 됐다'). ⚠️ `.sort-pool[hidden]{display:none}` 가드 필수 — `display:grid`가 UA `[hidden]`을 이긴다(`.door[hidden]`과 같은 함정).
- **③ 세 개의 문 = 타임라인**: **매일(브리핑룸→`briefing.html`) → 2주(챌린지→`challenges.html`) → 면접 직전(특강→`lectures.html`)**. 각 문 위에 `.tl-when` 라벨 + 앞 카드와 잇는 짧은 세로선(`.tl-step + .tl-step::before`)이 붙어 나열이 순서가 된다. **⚠️ 문 순서가 구 스케치(챌린지→브리핑룸→특강)와 다르다** — 되돌리면 ②의 그룹 순서·타임라인 라벨과 전부 어긋난다. 사진 카드가 좌우로 번갈아 붙는 리듬은 유지(2번째만 `rev`). **⚠️ 카드 자리는 반드시 사진**(텍스트 목록·미니 UI 덩어리는 오너 2회 기각). 사진 3장은 **샘플 — 최종본 오면 교체**. ⚠️ 특강은 `#doorLecture`(=`.tl-step`)가 `hidden` 시작, 공개 특강이 있을 때만 JS가 켠다 — **C안을 고른 이유 중 하나가 이것**으로, 문이 하나 빠져도 '매일 → 2주'만으로 순서가 성립한다. `.tl-step[hidden]`·`.door[hidden]` 가드 둘 다 유지. 문 칩은 **실데이터만**(브리핑룸 = 오늘 news_articles count, 특강 = 가장 임박한 신청중 특강 D-n) — 숫자를 꾸며내지 않는다.
...
- **⚠️ nav 활자 크기(2026-07-30 오너 "네비게이션 폰트가 너무 작다" → 약 50% 상향)**: 메뉴 14→**21px** · 신청하기 15→**21px** · 로그인 14→**18px** · 로고 22→**30px** · 드롭다운 15/13→18/15px. 자간 **-0.015em**(한글은 키울수록 자간이 벌어 보인다). 간격은 메뉴 gap 28→**26px**, 3컬럼 사이 16→28, 우측 두 버튼 사이 8→12. 세로 여백은 14→12px로 줄여 **바 높이 73px**(모바일은 57px 그대로). ⚠️ **CSS도 3파일(index.css·researchers.html·challenges.html) 동기화 대상**이다.
  - **⚠️⚠️ `.nav-inner`는 `auto 1fr auto`다 — `1fr auto 1fr`(메뉴를 화면 정중앙)로 되돌리지 말 것.** 오너 지적 "밸런스가 안 맞다"의 실체가 이것이었다: 왼쪽 로고 101px과 오른쪽 버튼 302px의 무게가 3배 차이라, 화면 기준으로 가운데를 잡으면 **왼쪽 225px · 오른쪽 28px**로 벌어져 메뉴가 버튼에 들러붙는다(실측). 가운데 트랙을 1fr로 두면 양쪽 여백이 저절로 같아진다(현재 151/151).
  - **⚠️ 메뉴 간격 26px + 행간 1**: 34px이면 글자 폭이 36px뿐인 '특강'·'후기'보다 틈이 넓어 메뉴가 다섯 덩어리로 흩어져 보인다. 행간을 안 잡으면 링크(body 1.7 상속, 높이 36px)와 드롭다운 버튼(26px)이 달라 **'브리핑룸'만 2px 처진다** — `.nav-links > li`를 flex로 세우고 링크·드롭다운 버튼에 `line-height: 1`을 준 이유다.
  - **⚠️ 로그인(18px)은 신청하기(21px)보다 한 단계 작다** — 같은 크기면 오른쪽 끝에 비슷한 알약 둘이 서서 어느 쪽이 본 행동인지 안 읽히고, 7자라 폭이 175px까지 벌어져 오른쪽이 무거워진다.
...
- **구성**: `<section class="hero-intro">` = `<picture>` 배경 한 장 + 정중앙 `.hi-stack`(문장 `.hi-phrase` ↔ 로고 `.hi-target`) + 한글 한 줄 + `#heroCue`(SCROLL). 높이 **100svh 한 장**.
- **안무**: 0~0.6초 문장 정지(읽는 시간) → 0.6~2.4초 **조립** → 2.35초 한글 → 2.9초 SCROLL. 총 3.6초, **로드 직후 rAF 1회**.
- **⚠️⚠️ 크기 = 화면 밖 문장 → 창에 꽉 찬 로고(2026-07-29 오너 "화면 밖으로 나갈 만큼 크게 했다가 창문 안으로 쭉 들어오는 것" + "마지막 로고가 너무 작다, 창문 안에 핏하게").** 문장 `clamp(84px, 22vw, 160px)`는 **일부러 화면보다 넓다**(실측 375px에서 화면의 **241%**, 1440px에서 **119%** — 좌우로 삐져나간다). 조립 배율 **0.60(모바일) ~ 0.41(웹)**.
- **⚠️⚠️ 로고 크기는 CSS 가 아니라 JS(`fitLogo`)가 창 폭에서 역산한다 — clamp 로 되돌리지 말 것.** 창의 화면 대비 폭이 **세로형 54% · 가로형 15.6%**로 완전히 달라 같은 `vw` 값이 두 경우에 전혀 다른 결과를 낸다. 게다가 배경이 `object-fit: cover` 라 화면 비율에 따라 이미지가 잘리면서 창 폭이 또 달라진다(실측: 1440×900 에서 창이 224px 이 아니라 **264px**). 그래서 cover 스케일을 직접 계산해 창 폭을 구하고 로고가 그 폭의 `FIT`(0.90)이 되도록 font-size 를 역산한다 — 실측 채움률 **88%**, 좌우 여백 12~15px. CSS 의 `clamp(40px, 11.5vw, 52px)`는 **JS 가 죽었을 때의 폴백일 뿐**이다.
- **⚠️ 개구부 비율은 이미지에서 실측한 상수**(세로형 가로 23.0~76.9% / 가로형 42.2~57.8%). **배경 이미지를 교체하면 이 값을 다시 재야 한다** — 안 재면 로고가 창을 벗어난다.
- **⚠️ 문장과 로고 크기는 한 쌍이다.** 로고만 창에 맞춰 키우고 문장을 그대로 두면 배율이 0.97 이 되어 **크기가 안 변하는 것처럼 보인다**(실제로 한 번 그렇게 됐다). 한쪽을 바꾸면 배율을 다시 볼 것.
- **⚠️ 이미지 로드 전에는 `naturalWidth`가 0이라 창 폭을 못 구한다** → `load` 이벤트에서 `measure()`를 다시 부른다. 이 재호출을 빼면 폴백 크기(작은 로고)로 굳는다.
- **⚠️ 화면보다 넓은 문장을 좌우 균등하게 넘기려면 `left:50%` + `translateX(-50%)` + `width:max-content` 여야 한다.** `left/right:0` + `text-align:center` 는 내용이 컨테이너보다 넓어지는 순간 왼쪽 기준이 되어 **오른쪽으로만 삐져나간다**(실측으로 밟은 자리).
...
- **⚠️ 한글 줄은 흐름에서 빼 `position:absolute; top:100%`로 둔다.** 흐름에 두면 그 줄 높이만큼 블록 중심이 내려가 **로고가 창 정중앙에서 벗어난다**(실측 49.1% vs 목표 52.6%). 지금은 모바일 50.0% / 웹 51.5%로 창 한가운데에 앉는다.
- **⚠️ `.hi-stack` 높이를 고정해 둔 이유** — 조립되며 글자가 커질 때 아래 한글 줄이 밀려 내려가는 걸 막는다.
- **⚠️ 웹폰트(SUIT)가 늦게 오면 글자 폭이 바뀐다** → `document.fonts.ready`에서 재측정한다. 리사이즈도 재생 중일 때만 재측정(끝난 뒤엔 `.hi-target`이 실물이라 무관).
