...
- **⚠️⚠️ 크기 = 화면 밖 문장 → 창 안 로고(2026-07-29 오너 "원래대로 화면 밖으로 나갈 만큼 크게 했다가 창문 안으로 쭉 들어오는 것").** 문장 `clamp(52px, 13vw, 150px)`는 **일부러 화면보다 넓다**(실측 375px에서 화면의 149%, 1440px에서 112% — 좌우로 삐져나간다). 로고 `clamp(40px, 11.5vw, 52px)`는 **창 개구부 안에 들어가는 크기**다(모바일 창 폭 202px에 로고 152px / 웹 224px에 184px). 그래서 조립은 **축소**다(배율 0.83~0.35). ⚠️ 문장을 줄이면 '밖에서 들어오는' 연출이 사라지고, 로고를 키우면 창틀을 넘어 '창 안으로 들어왔다'가 깨진다.
