...
| 기타 페이지(연구진·상세 5종·오디오) | `researchers.html`·`challenge-*.html`·`challenge-detail.css` | 상세 5종 인라인 공통 CSS 는 다섯 파일을 같이 고친다. ①안 '결과 먼저' 배치(2026-09-07 보신각 적용, 나머지 넷 순차)의 스타일은 `challenge-detail.css` 한 곳 — 인라인 복사 금지, 숫자는 실데이터 슬롯만 | `docs/notes/pages.md` |
