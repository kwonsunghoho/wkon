...
- **휴대폰 본인인증(2026-08-26, KG이니시스 통합인증·포트원 경유)**: 가입 온보딩 + **특강 신청·승준 도구·연구실 자료는 인증 회원만**(`MONC.requireVerified()` 게이트 — 도구 6·특강 3·연구실 2곳, 챌린지 신청·게임·뉴스는 대상 아님). 판정은 서버(verify-identity → `apply_identity_verification` service_role 전용), 인증 회원의 이름·번호는 화면 저장으로 못 바꾼다(save_my_profile 잠금). **게이트는 fail-open** — 안전장치(3중 + **인앱 브라우저 통과** 2026-08-27: 인앱은 인증창의 앱 전환이 막혀 게이트·온보딩 모두 인증을 강요하지 않는다)를 빼면 인프라 장애·인앱 유입이 전 기능 잠금이 된다. 채널 키는 `pay-methods.js` `identityChannel` 한 곳. 상세는 auth-consent.md '휴대폰 본인인증' 절. **승인된 예외 하나(2026-08-20 오너 확정)**: 회원 프로필 저장의 중복 가입 대조(`save_my_profile` RPC — 로그인 필수·24시간 10회 제한·기존 계정의 로그인 수단까지만 안내). 이 예외를 넓히려면 오너 확인 먼저.
