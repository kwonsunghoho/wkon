...
- **⚠️ 세로 기준은 '로고 중심 = 개구부 중심'이다 — 한글 줄까지 합친 덩어리의 중심이 아니다.** 한글 한 줄은 개구부(모바일 150px)보다 넓어(207px) 어차피 창틀을 가로지르는데, 로고를 정중앙에 두면 그 줄이 유리의 **아래쪽 흰 구름 구간**에 앉아 유리·창틀 경계가 글자 뒤에서 안 보인다. 덩어리 기준으로 올리면 한글이 파란 하늘 구간을 가로질러 경계선이 드러난다(두 안을 실제로 렌더해 비교한 결과).
