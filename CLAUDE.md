...
- `news.html` — **항공 뉴스 게시판(2026-07-22 배포).** 2탭(전체 뉴스 / 내 스크랩). 필터 = 제목 검색창 + 항공사 10개 칩 + 주제 5개 칩. ⚠️ 칩 줄은 모바일에서 **한 줄 가로 스크롤**(`flex-wrap:nowrap`+`overflow-x:auto`, ≥640px만 줄바꿈) — 세로로 쌓으면 sticky 필터바가 375px 화면의 70%를 먹어 기사가 안 보인다(실측). 스크랩 버튼은 **창문 심볼 + 글자 라벨 알약**(인라인 `#moncWin`, 전=`grayscale(1)`+'스크랩' / 후=원색+'저장됨'+코랄 틴트). 리본 북마크로 되돌리지 말 것이며 원본 SVG는 로고 교체(a5f1b85)로 삭제됐으므로 **인라인 심볼이 정본**이다. ⚠️ **라벨을 빼고 아이콘만 두지 말 것**(2026-07-22 오너 피드백 "저게 스크랩인지 모르겠다") — 창문은 브랜드 심볼이지 '저장'의 관습 기호가 아니라 아이콘 단독으로는 무슨 버튼인지 안 읽힌다. 시각 높이 36px에 `::after{inset:-4px}`로 터치 44px를 만들고, `.nw-link`의 `padding-right:98px`가 알약(≈75px) 자리를 비운다 — **알약은 absolute라 제목이 자동으로 안 밀린다. 라벨 길이를 바꾸면 이 여백도 같이 볼 것.**
...
### 랜딩 섹션 순서
창문 인트로 → 히어로 캐러셀(#home, `.hs-head` 이름표 '몬크 챌린지' + 카드별 모집 칩) → 연구진 신뢰 스트립(#researchers-strip) → 블라인드 퀴즈(#blind-quiz) → Before&After(#before-after) → 커뮤니티(#community) → 챌린지 진단(#advisor) → **브리핑룸 홍보(#briefing-home)** → 성장기록(#member-appeal) → 최종 CTA. 강사진 풀 섹션(#instructors)은 `researchers.html`로 분리(스트립이 티저). **구 "3단계 How"(#how)·"LIVE FEED" 후기 캐러셀은 삭제됨 — 재도입 금지**(진행방식 안내는 apply.html FAQ가 커버). 푸터 "서비스 소개"→`#home`. 스펙: docs/superpowers/specs/2026-07-12-landing-section-restructure.md, 2026-07-14-landing-text-diet.md.
...
- **⚠️ `.hs-cta`에 신청 버튼·D-day 뱃지를 두지 말 것(2026-07-22 오너 "신청하기가 굳이 있어야 해?"):** 현재 구성은 `[전·후 듣기 밑줄 링크] [상품 팩트 한 줄]`뿐이다. 구 구성 `[D-day 알약] [듣기 알약] [신청하기 알약] [팩트라인]`은 **코랄 알약 3개가 나란히 서서 무엇을 눌러야 하는지가 사라졌고**, 더 근본적으로 **하단 CTA 바가 상시 노출이라 100px 간격으로 '신청하기'가 두 번 뜨는 중복**이었다(스크린리더에도 같은 링크가 두 번 읽힌다). D-day·참가비도 바가 말하므로 히어로에선 뺀다.
  - **역할 분담(이걸 깨면 다시 지저분해진다):** 히어로 = *무엇인지*(카드) + *증거*(듣기 링크) + *상품 사실*(2주·미션·피드백) / 하단 바 = *신청* + *모집 상태*(D-day·참가비). **양쪽에 같은 정보를 두지 말 것.** (2026-07-23 보완: 카드 구석의 `.hs-status` 모집 칩은 예외 — 바는 '전체 중 가장 임박한 상태' 하나만 말하고, 칩은 '이 챌린지'의 상태를 말한다. #challenges 섹션 삭제 후 챌린지별 모집 정보의 유일한 자리라 중복이 아니다. recruit.js `applyIndexRecruit`가 채운다.)
  - 관련 CSS `.hs-cta-btn`·`.hs-cta-badge`는 2026-07-22 삭제(사용처 없음). 되살릴 땐 아래 무대 높이 상한도 +63px 할 것.
- **⚠️ `.hs-cta-proof`(2026-07-22 신설) — 원형 재생 버튼 + 2단 라벨:** `[44px 원 ▶] 전·후 목소리 듣기 / 10초면 압니다`. 목적지는 ③ Before&After 섹션. **밑줄 링크·알약으로 되돌리지 말 것** — 긴 문장 밑줄은 촌스럽고 재생 세모가 겉돌았으며(오너 지적), 알약은 하단 CTA 바와 위계가 겹친다. 동그란 재생 버튼이 설명 없이 '소리가 있다'를 말한다.
  - **통일감 계약(9대 원칙 — 하나라도 어기면 '뒤죽박죽'):** 원은 `.ba-play-btn`(전·후 섹션 재생 버튼)과 **같은 `--action` 채움 + `--action-ink` 아이콘 + 같은 path(`M8 5v14l11-7z`)**, 지름만 40→**44px**(터치 원칙). 활자는 **토큰만** — 라벨 `--fs-body`(17px)/800, 캡션 `--fs-caption`(14px)/600 `--text-dim`, 팩트 라인도 `--fs-caption`. **px 하드코딩 금지.** 간격은 `--space-*`(`.hs-cta` gap = `--space-2`). 삼각형은 `margin-left:2px` 광학 중심 보정.
...
- **⚠️ `.hs-stage` 높이의 세 번째 상한 제거 금지(2026-07-17 수정 · 2026-07-22 값 갱신):** `height: min(72svh, 600px, calc(max(100svh,680px) - 279px))` — **모바일 규칙(≤768px)에도 같은 세 번째 항이 필요**(구 `min(56svh,440px)`엔 없어서 375×812에서 `.hs-cta-sub`가 하단 바에 22px 깔렸다). 279 = 고정요소 180(nav 44 + CTA 블록 92 + 갭 48) + `.hs-carousel`의 `padding-bottom:80px` + 안전여유 19. **이 패딩은 하단 CTA 바(알약 58 + 바닥 14) 회피용** — `.hs-carousel`이 `justify-content:center`라 패딩이 없으면 콘텐츠 중심이 화면 중심에 맞춰지며 맨 아래 `.hs-cta-sub`가 바에 깔린다. **패딩과 279는 한 쌍이니 같이 움직일 것.** `.hs-cta`에 요소를 더할 때마다 그 높이+갭만큼 279를 올린다. (실측 2026-07-22: 375×812 무대 440·간격 62px, 1280×720 무대 441·간격 15px.) `.hs-carousel`은 `position:absolute; inset:0`이라 히어로 높이(`max(100svh,680px)`)에 **갇혀 못 자라고**, 자식(`.hs-stage`·`.hs-nav`·`.hs-cta`)은 전부 `flex:0 0 auto`라 **못 줄어든다.** 그런데 무대만 svh 비례고 화살표+갭+CTA는 **고정값**이라, 상한이 없으면 **창 높이 640~786px 구간에서 콘텐츠가 컨테이너를 24px 넘겨 `overflow:hidden`이 `.hs-cta-sub`를 반토막** 낸다(실측: 1280×700에서 필요 738 vs 박스 720). **노트북 창 높이 대역이라 데스크톱에서만 터지고 아이폰(812·844·852)은 멀쩡해 놓치기 쉽다.** 모집 중 `data-recruit-cta-badge`가 뜨면 +36px라 구간이 더 넓어진다(= 모집 없을 땐 재현이 안 된다). 무대가 줄어도 `.hs-card`는 `height:100%`라 사진 크롭만 더 되고 `.hs-card-content`는 안 잘린다. 큰 창(≥840px)에선 600px 상한이 이겨 **디자인 무변화**.
