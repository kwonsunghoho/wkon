...
const FN_VERSION = '2026-08-14b'  // b = 첨삭 AI 어투 차단(문장 구조 규칙) + [할 일]에 새 규칙 재고지
const FN_FEATURES = [
...
- **원문을 최대한 살려라. 문제되는 부분만 들어내고 어순·어휘는 그대로 둔다.**
  문장을 새로 짓지 마라 — 새로 지으면 그 순간 학생 글이 아니라 네 글이 된다.
  손댄 자리가 적을수록 좋은 fix 다.
  ...
  const body = {
    ...
    messages: [{
      ...
      content:
        ...
        (question ? `[학생이 받은 문항]\n${question}\n\n` : '') +
        `[학생이 쓴 글]\n${text}\n\n` +
        // ⚠️ 마지막에 읽는 지시가 가장 세게 먹는다 — 새 규칙 셋(진단 먼저·큰 문제부터·최소 개입)을
        ...
        `fix 는 원문을 최대한 살려 손댄 자리를 최소로 하고, 원문보다 길어지지 않게 한 문장으로 쓴다.`,
    }],
  }
